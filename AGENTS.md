...
- Every pressure service names its lever in `tests/fixtures/m3_policies.gd`, and `check.sh m3` requires the reference policy stripped of that lever, and the strongest lever-free policy the sweep found, to miss a target (`docs/specs/pressure-rebalance.md` §4.3).
