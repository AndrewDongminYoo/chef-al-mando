...
func run() -> void:
	...
	var settings := {"locale": locale, "sound_enabled": false, "text_size": "large" if large_text else "normal"}
	checks.expect(
		SettingsStore.new(settings_path).save_settings(settings).accepted, "service feedback capture settings are saved"
	)
	var scene: PackedScene = load("res://presentation/main.tscn")
	var screen := scene.instantiate() as KitchenScreen
	screen.scenario_path = (
		"res://content/campaign/scenarios/hot_queue.tres" if hot_queue else "res://content/m2_first_service.tres"
	)
	...
	checks.expect(
		screen.submit_preparation("set_prep", prep_mise_id, prep_quantity).accepted,
		"service feedback capture prepares the target portions"
	)
	if hot_queue:
		checks.expect(
			screen.submit_preparation("set_menu_priority", "grill", 2).accepted,
	...
	checks.expect(
		(
			chatter.get("kind", "") in ["order_wait_started", "prepared_stock_depleted", "order_ended"]
			and not chatter.get("text", "").is_empty()
		),
		"rendered service shows an evidence-based chef bubble"
	)
	await RenderingServer.frame_post_draw
	checks.expect(
		not screen.board.chatter_obscures_content(), "rendered chef bubble does not cover an employee or station"
	)
	checks.expect(
		(
			screen.safe_area.get_global_rect().encloses(screen.board.get_global_rect())
	...
	checks.expect(
		(
			screen.simulation.closed
			and screen.analysis_scroll.visible
...
func _expect_header_fits(screen: KitchenScreen, phase: String) -> void:
	...
	checks.expect(
		title_rect.size.x >= screen.title_label.get_combined_minimum_size().x,
		"rendered %s header keeps the full scenario title" % phase
	)
	checks.expect(
		(
			title_rect.end.x <= remaining_rect.position.x
	...
	checks.expect(
		screen.safe_area.get_global_rect().encloses(settings_rect),
...
func _expect_analysis_action_style(screen: KitchenScreen) -> void:
	...
	var action_text := label.get_parsed_text().substr(action_range.x, action_range.y - action_range.x).trim_suffix("\n")
	...
	var expected_width := (
		...
		. get_string_size(action_text, HORIZONTAL_ALIGNMENT_LEFT, -1, expected_font_size)
		. x
	)
	checks.expect(
		absf(label.get_line_width(action_line) - expected_width) < 2.0,
		"rendered analysis action uses the observation font size"
	)
	RenderingServer.force_draw(false)
	var frame := root.get_texture().get_image()
	var line_rect := Rect2(
		label.get_global_rect().position + Vector2(0, label.get_line_offset(action_line)),
		Vector2(label.size.x, label.get_line_height(action_line))
	)
	checks.expect(
		_region_contains_color(frame, line_rect, expected_color),
...
func _expect_korean_analysis_line_groups(screen: KitchenScreen) -> void:
	...
	for phrase: String in [
		...
		checks.expect(
			start >= 0 and label.get_character_line(start) == label.get_character_line(start + phrase.length() - 1),
			"rendered Korean analysis keeps the semantic group on one line: " + phrase.replace("\u2060", "")
		)
...
func _region_contains_color(frame: Image, region: Rect2, target: Color) -> bool:
	...
	for pixel_y: int in range(start.y, finish.y):
		for pixel_x: int in range(start.x, finish.x):
			...
			if absf(pixel.r - target.r) < 0.08 and absf(pixel.g - target.g) < 0.08 and absf(pixel.b - target.b) < 0.08:
				return true
	return false
