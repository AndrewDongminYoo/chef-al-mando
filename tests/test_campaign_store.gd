...
func run(_tree: SceneTree) -> void:
	...
	var records: Dictionary = {
		scenario.id: {"completed": true, "best_served": scenario.minimum_served, "best_profit": scenario.minimum_profit}
	}
	...
	expect(
		loaded.accepted and loaded.records.is_empty() and loaded.reason == "new_campaign",
		"missing primary and backup start a new campaign"
	)
	...
	expect(
		int(backup_document.records[scenario.id].best_served) == scenario.minimum_served,
		"backup contains the previous valid record"
	)
	expect(
		CampaignStore.new(campaign, file_path).load_records().records == improved,
		"new primary contains the improved record"
	)
	...
	for failure: String in ["write", "invalid_temp", "backup", "replace"]:
		...
		expect(
			FileAccess.get_file_as_bytes(file_path) == primary_bytes,
			"failed save preserves the primary bytes: " + failure
		)
		expect(
			CampaignStore.new(campaign, file_path).load_records().records == improved,
			"failed save remains readable from a new store: " + failure
		)
		failing.failure = ""
		if failure == "replace":
	...
	expect(
		not loaded.accepted and loaded.reason == "future_version" and not loaded.can_recover,
		"future primary versions block recovery"
	)
	expect(
		not store.save_records(records).accepted and not store.recover_backup().accepted,
		"save and backup recovery cannot overwrite future data"
	)
	...
	loaded = CampaignStore.new(campaign, file_path).load_records()
	expect(
		not loaded.accepted and loaded.can_recover and loaded.records.is_empty(),
		"corruption offers recovery without silently loading the backup"
	)
	...
	expect(
		not failed_recovery.recover_backup().accepted and FileAccess.get_file_as_string(file_path) == "{broken",
		"failed explicit recovery preserves the corrupt primary"
	)
	...
	expect(
		...
		"a new store reads the explicitly recovered record"
	)
	...
	for invalid_kind: String in [
		...
		match invalid_kind:
			...
			"skipped":
				...
				invalid.records = {
					second.id:
					{"completed": true, "best_served": second.minimum_served, "best_profit": second.minimum_profit}
				}
			"extra_field":
		...
		expect(
			not store.save_records(records).accepted and FileAccess.get_file_as_bytes(file_path) == original,
			"invalid records are not silently reset: " + invalid_kind
		)
