...
func run(tree: SceneTree) -> void:
	...
	expect(
		summary.text.contains("6,600") and summary.text.contains("샐러드"),
		"preparation shows the actual purchase cost and menus"
	)
	...
	expect(
		sim.call("snapshot").orders[0].priority == 1 and sim.call("snapshot").commands.size() == 1,
		"a priority tap while paused queues one command"
	)
	expect(screen.get("feedback_label").text.contains("적용 대기"), "paused commands display their pending state")
	expect(
		(
			screen.get("detail_label").text.contains("우선순위 2")
			and screen.get("order_buttons").order_01.text.contains("우선 2")
		),
		"paused priority taps immediately update both displayed priority values"
	)
	...
	screen.call("advance", 0.1)
	expect(
		sim.call("snapshot").orders[0].state == "cancelled" and sim.call("snapshot").employees[0].duty == "off",
		"cancel and duty changes reach the simulation"
	)
	...
	expect(
		screen.get("detail_panel").visible and not screen.get("details_toggle").visible,
		"tablet layout keeps details visible in parallel"
	)
	...
	expect(
		sim.get("closed") and screen.get("restart_button").visible, "closing exposes restart on the real result screen"
	)
	expect(
		screen.get("summary_label").text.contains("손익") and screen.get("summary_label").text.contains("폐기"),
		"closing displays accounting and waste"
	)
	screen.get("restart_button").pressed.emit()
	var restarted: RefCounted = screen.get("simulation")
	expect(
		restarted != sim and restarted.get("tick") == 0 and restarted.call("snapshot").orders.is_empty(),
		"restart creates a fresh prepared service"
	)
	expect(
		restarted.call("snapshot").inventory.vegetable == 22 and screen.get("driver").paused,
		"restart restores inventory and stays paused"
	)
	screen.get("start_button").pressed.emit()
	screen.call("advance", 1.0)
	screen.get("pause_button").pressed.emit()
	...
	screen.get("priority_up_button").pressed.emit()
	expect(
		restarted.call("snapshot").commands.size() == 2,
		"the rejection fixture queues cancellation before priority on the same order"
	)
	screen.get("resume_button").pressed.emit()
	screen.call("advance", 0.2)
	expect(
		restarted.call("snapshot").orders[0].state == "cancelled",
		"the rejection fixture executes cancellation across multiple ticks"
	)
	expect(
		screen.get("feedback_label").text.contains("적용하지 못했습니다"),
		"a rejected command remains visible after later ticks in the same frame"
	)
	var data: Resource = ResourceLoader.load(
	...
	expect(
		screen.get("start_button").disabled and screen.get("summary_label").text.contains("오류"),
		"the screen presents invalid content as an error and disables start"
	)
...
func _test_priority_press_survives_refresh(tree: SceneTree, speed: int) -> void:
	...
	expect(
		view.commands.size() == 1 and view.commands[0].kind == "set_priority" and view.commands[0].value == 2,
		"release after service ticks submits exactly one priority command"
	)
	expect(
		(
			screen.get("detail_label").text.contains("우선순위 2")
		...
		"queued priority appears in the selected detail and order row before the next tick"
	)
	screen.call("advance", 0.1)
	expect(
		screen.get("simulation").snapshot().orders[0].priority == 2,
		"the held priority tap reaches the simulation on the next tick"
	)
	screen.queue_free()
	await tree.process_frame
...
func _test_reordered_employees(tree: SceneTree) -> void:
	...
	expect(
		sim.call("snapshot").commands[0].target_id == displayed.id,
		"a duty control targets its displayed employee after resource reordering"
	)
	screen.get("start_button").pressed.emit()
	screen.call("advance", 0.1)
	var employees: Array = sim.call("snapshot").employees
	expect(
		employees[0].duty == "cold" and employees[1].duty == "all",
		"a duty change applies only to the displayed employee after resource reordering"
	)
	screen.queue_free()
	await tree.process_frame
	data.get("employees").reverse()
