...
func run(tree: SceneTree) -> void:
	...
	for invalid: Variant in [-1, 3, 1.5, "2", true, null]:
		...
		expect(
			not _command(plan, "set_menu_priority", "grill", invalid).accepted,
			"preparation rejects invalid priority: " + str(invalid)
		)
	...
	expect(
		PreparationPlan.new(scenario, legacy).snapshot().menu_priorities.grill == 1,
		"legacy preparation defaults to one"
	)
	...
	if normal.accepted and fast.accepted:
		expect(
			(
				normal.snapshot.accounting.served >= scenario.minimum_served
				and normal.snapshot.accounting.profit >= scenario.minimum_profit
			),
	...
	expect(
		(
			closer_result.accepted
			and (
				closer_result.snapshot.accounting != normal.snapshot.accounting
...
func _command(plan: RefCounted, kind: String, target: String, value: Variant) -> Dictionary:
	return plan.apply_command(
		{"kind": kind, "target_id": target, "value": value, "apply_tick": 0, "sequence": plan.snapshot().sequence + 1}
	)
...
func _test_restore(campaign: Resource) -> void:
	...
	expect(
		sim.state_hash() == restored.simulation.state_hash(),
		"resumed future arrivals retain menu priorities to closing"
	)
			...
			. enqueue_command(
				{
					...
					"target_id": first_id,
					"value": 2,
					"apply_tick": changed_sim.tick + 1,
					"sequence": 1
				}
			)
			. accepted
		),
		"an order override remains available"
	)
	changed_sim.step()
	...
	expect(
		(
			restored_override.accepted
			and restored_override.simulation.snapshot().orders[0].priority == 2
	...
	for invalid: Variant in [
		{}, {"salad": 3}, {"salad": -1}, {"salad": true}, {"salad": 1.5}, {"salad": "2"}, {"salad": 1, "unknown": 2}
	]:
		var bad := session.duplicate(true)
		bad.preparation.menu_priorities = invalid
		expect(
			not ServiceSession.restore(campaign, bad, {}).accepted,
			"stored priorities reject incomplete or invalid menu values: " + str(invalid)
		)
	var forged := session.duplicate(true)
	forged.simulation.orders[0].priority = 1
	expect(
		not ServiceSession.restore(campaign, forged, {}).accepted,
		"a priority inconsistent with a command-free menu default is rejected"
	)
	...
	expect(
		loaded.accepted and FileAccess.get_file_as_bytes(target) == bytes,
		"schema 2 loads without rewriting old save bytes"
	)
	if loaded.accepted:
		var old_restored := ServiceSession.restore(campaign, loaded.active_session, {})
		expect(
			old_restored.accepted and old_restored.simulation.state_hash() == old_sim.state_hash(),
			"legacy session retains exact simulation state"
		)
	...
	expect(
		loaded.accepted and ServiceSession.restore(campaign, loaded.active_session, {}).accepted,
		"a fresh store reads saved menu defaults"
	)
	...
	for speed: int in [1, 4]:
		...
		for choice: Dictionary in policy.preparation:
			if choice.kind != "set_menu_priority":
				expect(
					screen.call("submit_preparation", choice.kind, choice.target_id, choice.value).accepted,
					"UI accepts reference preparation"
				)
		...
		await tree.process_frame
		expect(
			panel.get("pages")[0].get_global_rect().encloses(button.get_global_rect()),
			"the actual menu priority button is inside the scroll viewport"
		)
		...
		await tree.process_frame
		expect(
			screen.get("preparation").snapshot().menu_priorities.grill == 2 and button.disabled,
			"pointer release sets priority two and disables the upper bound"
		)
		expect(
			panel.get("priority_labels").grill.text.contains("우선순위 2"),
			"the preparation screen displays the selected priority"
		)
		...
		expect(
			(
				view.closed
				and view.accounting.served >= scenario.minimum_served
