...
func command(plan: RefCounted, kind: String, target: String, value: Variant, sequence: int) -> Dictionary:
	return plan.call(
		"apply_command", {"kind": kind, "target_id": target, "value": value, "apply_tick": 0, "sequence": sequence}
	)
...
func _test_conversion_and_commit() -> void:
	...
	expect(
		prepared.inventory.vegetable == 18 and prepared.inventory.grain == 6 and prepared.inventory.prepped_soup == 2,
		"soup preparation converts two complete raw portions"
	)
	expect(
		prepared.labor_used == 4 and prepared.purchased_cost == 6600,
		"preparation spends labor without adding a second cash cost"
	)
	...
	expect(
		(
			prepared.inventory.vegetable == 20
			and prepared.inventory.grain == 7
			and prepared.inventory.prepped_soup == 1
			and prepared.labor_used == 2
		),
		"reducing preparation restores only the preview inputs"
	)
	var result := command(plan, "start", "", null, 3)
	expect(result.accepted and view(plan).committed, "a valid preparation plan commits once")
	expect(
		result.inventory.prepped_soup == 1 and result.inventory.vegetable == 20,
		"the committed inventory matches the selected conversion"
	)
	expect(not command(plan, "start", "", null, 4).accepted, "a repeated start cannot convert ingredients twice")
	expect(
		not command(plan, "set_prep", "prepped_soup", 0, 4).accepted, "preparation cannot change after service starts"
	)
	expect(
		data.purchases.vegetable == 22 and data.purchased_cost() == 6600,
		"committing does not consume the source resource"
	)
...
func _test_budget_and_invalid_commands() -> void:
	...
	expect(
		not command(plan, "set_purchase", "protein", 12, 1).accepted,
		"purchases cannot consume the fixed labor allowance"
	)
	expect(command(plan, "set_purchase", "protein", 11, 1).accepted, "a purchase below the combined budget is allowed")
	expect(
		command(plan, "set_purchase", "vegetable", 24, 2).accepted, "purchases plus labor may equal the budget exactly"
	)
	expect(view(plan).budget_remaining == 0, "the exact budget boundary has no uncommitted money")
	for invalid: Array in [
		["set_purchase", "vegetable", 25],
		["set_purchase", "vegetable", -1],
		["set_purchase", "protein", 9223372036854775807],
		["set_purchase", "vegetable", "24"],
		["set_purchase", "prepped_salad", 1],
		["set_prep", "unknown", 1],
		["set_prep", "prepped_salad", -1],
		["set_duty", "employee_01", "unknown"],
		["set_duty", "unknown", "all"],
		["move_station", "storage_01", "diagonal"],
		["unknown", "", null],
	]:
		var before := JSON.stringify(view(plan), "", true)
		expect(
			not command(plan, invalid[0], invalid[1], invalid[2], 3).accepted,
			"invalid preparation command is rejected: " + str(invalid)
		)
	...
	expect(
		command(partial_plan, "set_purchase", "vegetable", 20, 1).accepted,
		"a scenario that omits a purchasable ingredient still accepts its authored purchases"
	)
	expect(
		(
			command(partial_plan, "set_purchase", "protein", 1, 2).accepted
	...
	expect(
		ServiceSession._exact_preparation(partial, view(partial_plan).selection),
...
func _test_shared_raw_stock() -> void:
	...
	expect(
		not command(plan, "start", "", null, 2).accepted and not view(plan).committed,
		"an unsellable menu blocks service without committing"
	)
	expect(
		not command(plan, "set_prep", "prepped_salad", 1, 2).accepted,
		"different preparations cannot spend the same vegetable"
	)
	expect(
		not command(plan, "set_purchase", "vegetable", 1, 2).accepted,
		"purchases cannot fall below the selected preparation inputs"
	)
	expect(
		command(plan, "set_purchase", "vegetable", 3, 2).accepted,
		"purchasing one more vegetable restores menu coverage"
	)
	expect(
		view(plan).can_start and view(plan).inventory.vegetable == 1,
		"coverage uses the remaining raw and prepared inventory"
	)
...
func _test_raw_menu_coverage() -> void:
	for reverse_menus: bool in [false, true]:
		...
		var before := JSON.stringify(view(plan), "", true)
		expect(
			not view(plan).can_start and "menu_missing_ingredients" in view(plan).errors,
			"raw menus cannot reuse the same vegetable for minimum coverage"
		)
		var rejected := command(plan, "start", "", null, 1)
		expect(
			not rejected.accepted and rejected.reason == "menu_missing_ingredients",
			"shared raw stock shortage rejects service start"
		)
		expect(
			JSON.stringify(view(plan), "", true) == before,
			"coverage rejection preserves inventory, choices, sequence, and commit state"
		)
		expect(
			command(plan, "set_purchase", "vegetable", 3, 1).accepted and view(plan).can_start,
			"exact combined raw stock covers every menu in either order"
		)
		var started := command(plan, "start", "", null, 2)
		expect(
			(
				started.accepted
				and started.inventory.vegetable == 3
				and started.inventory.grain == 1
				and started.inventory.protein == 1
			),
			"coverage validation does not consume the committed inventory"
		)
...
func _test_placement() -> void:
	...
	expect(
		command(plan, "move_station", "pass_01", "left", 1).accepted, "the approved closer pass position is reachable"
	)
	expect(
		_station(view(plan), "pass_01").tile == [8, 5] and _station(view(plan), "pass_01").work_position == [8, 4],
		"movement carries the station work position with it"
	)
	expect(
		command(plan, "rotate_station", "pass_01", null, 2).accepted, "a clockwise work position rotation is accepted"
	)
	...
	expect(
		not command(plan, "move_station", "cold_01", "left", 1).accepted,
		"closing the only corridor must reject a globally disconnected kitchen"
	)
	plan = plan_script.new(fresh())
	expect(
		command(plan, "move_station", "storage_01", "down", 1).accepted,
		"the first downward move keeps the employee start open"
	)
	expect(
		command(plan, "move_station", "storage_01", "down", 2).accepted,
		"a work position may share an employee starting tile"
	)
	expect(
		not command(plan, "move_station", "storage_01", "down", 3).accepted,
		"a station may not block an employee starting tile"
	)
...
func _test_source_isolation_and_restart() -> void:
	...
	expect(
		_station(view(second), "pass_01").tile == [9, 5] and view(second).inventory.prepped_grill == 0,
		"preparing one session does not change another session"
	)
	var repeated: RefCounted = plan_script.new(data, committed.selection)
	expect(
		(
			view(repeated).inventory.protein == 6
			and view(repeated).inventory.prepped_grill == 2
			and view(repeated).duties.employee_01 == "hot"
		),
		"retry rebuilds fresh inventory from the last preparation choices"
	)
	expect(
		not view(repeated).committed and view(repeated).sequence == 0 and view(repeated).purchased_cost == 6600,
		"retry resets the service boundary without carrying revenue or loss"
	)
	var exposed := view(repeated)
	exposed.inventory.protein = -999
	exposed.stations[0].tile[0] = -999
	expect(
		view(repeated).inventory.protein == 6 and view(repeated).stations[0].tile[0] != -999,
		"display snapshots cannot mutate preparation state"
	)
	expect(command(repeated, "reset", "", null, 1).accepted, "the default preparation can be restored explicitly")
	expect(
		(
			view(repeated).inventory.protein == 8
			and view(repeated).inventory.prepped_grill == 0
			and _station(view(repeated), "pass_01").tile == [9, 5]
		),
		"default reset restores source purchases and layout"
	)
...
func _test_invalid_content() -> void:
	...
	var plan: RefCounted = plan_script.new(data)
	expect(
		not view(plan).can_start and not view(plan).errors.is_empty(),
		"malformed content returns a rejected preparation state"
	)
