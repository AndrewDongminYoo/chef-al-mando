...
func run() -> void:
	...
	checks.expect(
		touch_actions == 1 and not screen.is_running(), "one touch pauses exactly once (actions=%d)" % touch_actions
	)
	checks.expect(
		screen.get("input_actions") == 4, "the installed diagnostic reader counts each rendered button action once"
	)
