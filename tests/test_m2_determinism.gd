...
func run(_tree: SceneTree) -> void:
	...
	expect(
		first.state_hash() == repeat.state_hash(),
		"M2 preparation, layout, duties, and service commands repeat the final hash"
	)
	expect(
		first.state_hash() == fast.state_hash(), "M2 real tick drivers preserve the same state at one and four speed"
	)
...
func _prepared_replay(speed: int, frames: Array[int]) -> ServiceSim:
	...
	expect(
		sim.tick == 3000 and driver.get("accumulator_us") == 0, "M2 driver reaches closing without dropping game time"
	)
	return sim


func _prep_command(plan: PreparationPlan, kind: String, target: String, value: Variant, sequence: int) -> Dictionary:
	var result := plan.apply_command(
...
func _measure_choices() -> void:
	...
	for policy: String in ["baseline", "prep", "placement", "priority", "combined"]:
					...
					for order: Dictionary in sim.snapshot().orders:
						if order.id == event.order_id and order.recipe_id == "grill":
							...
							expect(
								...
								"comparison priority is applied one tick after grill arrival"
							)
		var view := sim.snapshot()
		expect(
			view.orders.size() == 20 and view.tick == 3000 and view.accounting.purchased_cost == 6600,
			"comparison preserves twenty orders, closing time, and purchases"
		)
		...
		for order: Dictionary in view.orders:
			...
			expect(
				elapsed == order.ended_tick - order.arrival_tick,
				"comparison order times partition residence: " + policy + "/" + order.id
			)
			order_times.append({"id": order.id, "ended_tick": order.ended_tick, "state": order.state})
		runs[policy] = {
			"accounting": view.accounting, "metrics": view.metrics, "order_times": order_times, "hash": sim.state_hash()
		}
		print("M2_COMPARISON ", policy, " ", JSON.stringify(runs[policy], "", true))
	for policy: String in ["prep", "placement", "priority"]:
		expect(
			runs[policy].metrics != runs.baseline.metrics, "the fixed comparison measures a time effect for " + policy
		)
	var outcome_changed: bool = false
	for policy: String in ["prep", "placement", "priority", "combined"]:
		if (
			runs[policy].accounting.served != runs.baseline.accounting.served
			or runs[policy].accounting.profit != runs.baseline.accounting.profit
		):
...
func _test_rotation_changes_service_route() -> void:
	...
	expect(
		original.path_size == 7 and rotated.path_size == 9,
		"a committed pass rotation changes the actual serving route from six to eight edges"
	)
	expect(
		rotated.moving_ticks == original.moving_ticks + 10,
		"two added serving edges consume ten additional movement ticks"
	)
...
func _placement_run(rotated: bool) -> Dictionary:
	...
	while not placement_sim.closed:
		...
		if (
			not view.orders.is_empty()
			and view.orders[0].phase_id == "serve"
			and view.orders[0].state == "moving"
			and path_size == 0
		):
			path_size = view.tasks[0].path.size()
	var order: Dictionary = placement_sim.snapshot().orders[0]
	expect(
		order.state == "served" and path_size > 0,
		"the rotation fixture must reach a moving serving task and complete service"
	)
