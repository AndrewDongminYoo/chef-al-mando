...
func run(tree: SceneTree) -> void:
	...
	await _expect_row_drag_scrolls(
		tree, screen.get("list_scroll"), screen.get("scenario_buttons").first_shift, "campaign service"
	)
	...
	await tree.process_frame
	expect(
		service.get("definitions").id == "first_shift",
	...
	expect(
		not service.call("is_running") and screen.get("leave_dialog").visible,
		"leaving a running service pauses it and asks before discard"
	)
	...
	expect(
		screen.get("last_result").passed and screen.get("result_dialog").visible,
		"actual closing displays a passing result"
	)
	...
	expect(
		FileAccess.get_file_as_bytes(file_path) == bytes, "a duplicate closing signal does not change the saved record"
	)
	screen.get("retry_service_button").pressed.emit()
	expect(
		service.get("simulation").tick == 0 and service.get("state") == 0,
		"retry returns the current service to preparation"
	)
	expect(
		service.get("definitions").starting_budget == screen.get("campaign").scenarios[0].starting_budget,
		"retry restores the scenario budget"
	)
	...
	expect(
		not screen.get("last_result").passed and screen.get("next_button").disabled,
	...
	await tree.process_frame
	expect(
		not screen.get("scenario_buttons").lunch_prep.disabled,
		"the previously unlocked service remains available after failure"
	)
	screen.queue_free()
	await tree.process_frame
	...
	await tree.process_frame
	expect(
		not screen.get("scenario_buttons").lunch_prep.disabled, "restarting the actual scene reloads its saved unlock"
	)
	...
	for index: int in range(1, campaign.scenarios.size()):
		...
		if scenario.id == "hot_queue":
			expect(
				service.get("summary_label").text.contains("주문 0 / 20건"),
				"the running summary distinguishes arrived orders from the full schedule"
			)
		for arrival: Dictionary in scenario.order_schedule():
			service.call(
				"advance",
				(arrival.arrival_tick - service.get("simulation").tick) / (10.0 * service.get("driver").speed)
			)
			if scenario.id == "hot_queue" and arrival.id == "order_01":
				expect(
					service.get("summary_label").text.contains("주문 2 / 20건"),
					"the running summary counts the first order batch before any order is served"
				)
			if policy.priorities.has(arrival.recipe_id):
				if scenario.id == "hot_queue":
					await _tap_hot_queue_priority(tree, service, arrival.id)
				else:
					var changed: Dictionary = service.call(
						"submit_command", "set_priority", arrival.id, policy.priorities[arrival.recipe_id]
					)
		...
		if scenario.id == "hot_queue":
			...
			expect(
				view.orders.size() == 20 and view.accounting.expired > 0 and not view.closed,
				"the hot-queue fixture includes all arrivals and real expired orders before closing"
			)
			await _expect_row_drag_scrolls(
				tree,
				service.get("order_buttons").order_10.get_parent().get_parent(),
				service.get("order_buttons").order_10,
				"service order"
			)
			var counts: Array = [
				view.orders.size(),
				scenario.order_count,
				view.accounting.served,
				view.accounting.expired,
				view.accounting.cancelled
			]
			expect(
				service.get("summary_label").text.contains("주문 %d / %d건 · 제공 %d · 미제공 %d · 취소 %d" % counts),
		...
		expect(
			(
				screen.get("result_dialog").dialog_text.contains("제공")
				and screen.get("result_dialog").dialog_text.contains("손익")
			),
			"result text shows both measured target values"
		)
		if index == 1 or index == campaign.scenarios.size() - 1:
			...
			expect(
				(
					screen.get("result_dialog").visible
					and not screen.get("next_button").disabled
					and screen.get("next_button").text == korean_action
				),
			...
			expect(
				(
					screen.get("result_dialog").visible
					and not screen.get("next_button").disabled
					and service.get("simulation").state_hash() == result_hash
				),
				(
					"result locale changes preserve dialog visibility, action availability, and simulation state: "
					+ scenario.id
				)
			)
		if index == 5:
	...
	expect(
		...
		"the final next action displays the ending and leaves service"
	)
	screen.queue_free()
	await tree.process_frame
	screen = _boot(tree, entry, file_path)
	...
	await tree.process_frame
	expect(
		screen.get("begin_button").disabled and not screen.get("recover_button").visible,
		"future records block normal start and unsafe recovery"
	)
	screen.get("session_only_button").pressed.emit()
	expect(
		not screen.get("begin_button").disabled and screen.get("save_label").text.contains("저장 없이"),
		"explicit session-only play explains that results will not be saved"
	)
	screen.get("begin_button").pressed.emit()
	service = screen.get("active_service")
	service.set_process(false)
	...
	service.call("advance", 300.0)
	expect(
		FileAccess.get_file_as_bytes(file_path) == bytes, "session-only completion preserves the future record bytes"
	)
	screen.queue_free()
	await tree.process_frame
...
func _expect_row_drag_scrolls(tree: SceneTree, scroll: ScrollContainer, row: Button, context: String) -> void:
	...
	await tree.process_frame
	expect(
		scroll.get_global_rect().has_point(row.get_global_rect().get_center()),
		context + " row center is visible inside its scroll viewport"
	)
	var activations := {"count": 0}
	var drag_events := {"count": 0}
	row.pressed.connect(func() -> void: activations.count += 1)
	scroll.gui_input.connect(
...
func _tap_hot_queue_priority(tree: SceneTree, service: Control, order_id: String) -> void:
	...
	expect(
		...
		"the input fixture exposes the actual order row in its scroll viewport"
	)
	...
	for button: Button in [row, service.get("priority_up_button")]:
		...
		expect(
			service.get("simulation").tick == before_tick + 2 * service.get("driver").speed and button.is_pressed(),
			"the held pointer survives the real service ticks at the selected speed"
		)
	...
	expect(
		service.get("command_sequence") == sequence_before + 1, "one actual priority tap produces exactly one command"
	)
	expect(
		service.get("detail_label").text.contains("우선순위 2") and row.text.contains("우선 2"),
		"the actual priority tap displays its queued value immediately"
	)
...
func _save_failure_navigation(tree: SceneTree, entry: String, file_path: String) -> void:
	...
	service.call("advance", 300.0)
	expect(
		screen.get("last_result").passed and screen.get("pending_save"),
		"a real passing service reaches the failed save path"
	)
	expect(
		(
	...
	for action: String in ["next", "retry", "menu", "direct_menu"]:
		...
		expect(
			screen.get("active_service") == service and service.get("simulation").tick == 3000,
			"pending save preserves the closed service after navigation: " + action
		)
		expect(
			(
				not screen.get("last_result").is_empty()
				and screen.get("result_dialog").visible
				and screen.get("retry_save_button").visible
			),
			"pending save keeps the result and retry action available: " + action
		)
	screen.get("retry_save_button").pressed.emit()
	expect(
		screen.get("pending_save") and screen.get("next_button").disabled,
		"another save failure keeps navigation blocked"
	)
	store.failure = ""
	screen.get("retry_save_button").pressed.emit()
	expect(
	...
	expect(
		reopened.load_records().records == screen.get("progress").snapshot().records,
