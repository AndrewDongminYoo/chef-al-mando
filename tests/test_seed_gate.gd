...
func _test_draw_aware_policy(campaign: Resource) -> void:
	for scenario: Resource in campaign.scenarios:
		...
		expect(
			purchases == expected,
			(
				"seed 0 draw-aware purchases equal the authored purchases overlaid by the reference purchase commands: "
				+ scenario.id
			)
		)
		...
		expect(
			(
				policy.preparation.slice(purchases.size()) == behind.preparation
				and policy.priorities == reference.priorities
			),
	...
	expect(
		seeded_purchases == expected_purchases,
...
func _test_synthetic_failures(campaign: Resource) -> void:
	...
	expect(
		(
			not verdict.passed
			and verdict.failures.size() == ATTEMPTS.size()
			and verdict.failures[0].begins_with("attempt 1:")
			and verdict.failures[0].contains("misses the targets")
		),
	...
	expect(
		(
			not verdict.passed
			and verdict.failures.size() == 1
			and verdict.failures[0].begins_with("attempt 0:")
			and verdict.failures[0].contains("no plan misses by less than")
		),
...
func _test_campaign_gate(campaign: Resource) -> void:
	for scenario: Resource in campaign.scenarios:
		...
		expect(
			verdict.passed,
			"attempts 1-5 are solvable and pressured: %s %s" % [scenario.id, ", ".join(verdict.failures)]
		)
		for attempt: int in ATTEMPTS:
			...
			if scenario.forecast_slack.is_empty():
				expect(
					ScheduleGenerator.recipe_ids(scenario, seed_value) == scenario.order_recipe_ids,
