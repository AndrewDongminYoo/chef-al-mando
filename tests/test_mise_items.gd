...
const CAMPAIGN_NAMES := {
	...
	"salad": "토마토 샐러드",
	"grain_salad": "현미 샐러드",
	"mushroom_salad": "양송이 샐러드",
	"soup": "토마토 수프",
	"mushroom_soup": "양송이 수프",
	"grain_grill": "현미 볶음밥"
}
...
func _test_mise_definitions() -> void:
	...
	expect(
		_ingredient(data, "prepped_salad").call("is_mise") and not _ingredient(data, "vegetable").call("is_mise"),
	...
	expect(
		item_ids == ["prepped_salad", "prepped_soup", "prepped_grill"], "mise_items keeps the ingredient array order"
	)  # content/m2_first_service.tres:237
	expect(
		data.call("menu_count_for", "prepped_salad") == 1 and data.call("menu_count_for", "vegetable") == 0,
	...
	expect(
		_has_error(wrong_cost, "mise cost must equal its raw input cost: prepped_soup"),
		"mise cost must equal the raw input cost"
	)

	var raw_with_inputs := _fixture()
	_ingredient(raw_with_inputs, "vegetable").set("inputs", _typed_inputs({"grain": 1}))
	expect(
		_ingredient(raw_with_inputs, "vegetable").get("inputs") == _typed_inputs({"grain": 1}),
		"the typed set lands on the fixture"
	)
	expect(
		_has_error(raw_with_inputs, "raw ingredients cannot have inputs or labor: vegetable"),
		"raw ingredients cannot carry inputs"
	)
	...
	expect(
		_has_error(negative_labor, "mise items need inputs and labor: prepped_grill"),
		"a mise item rejects negative labor"
	)
	...
	expect(
		_has_error(mismatched_inputs, "recipe ingredients must equal the raw inputs of its mise items: soup"),
	...
	expect(
		"campaign menu needs mise items: salad" in empty_menu.call("validate"),
		"a campaign menu without mise items is rejected"
	)
	expect(
		"unused mise item: prepped_vegetable" in empty_menu.call("validate"),
		"a mise item no sale menu uses is rejected"
	)
...
func _command(plan: PreparationPlan, kind: String, target: String, value: Variant, sequence: int) -> Dictionary:
	return plan.apply_command(
		{"kind": kind, "target_id": target, "value": value, "apply_tick": 0, "sequence": sequence}
	)
...
func _test_mise_preparation() -> void:
	...
	expect(
		plan.snapshot().prep_quantities.keys() == ["prepped_salad", "prepped_soup", "prepped_grill"],
		"preparation quantities are keyed by mise item in ingredient order"
	)  # content/m2_first_service.tres:237
	...
	expect(
		(
			snapshot.labor_used == 4
			and snapshot.inventory.prepped_soup == 2
			and snapshot.inventory.vegetable == 18
			and snapshot.inventory.grain == 6
		),
		"mise labor and raw inputs come from the item definition"
	)
	...
	expect(
		(
			PreparationPlan.missing_mise_ids(inventory, plan.display_definition().recipe_for("salad")).is_empty()
			and (
				PreparationPlan.missing_mise_ids(inventory, plan.display_definition().recipe_for("soup"))
				== ["prepped_soup"]
			)
		),
	...
	expect(
		report.prep.keys() == ["prepped_salad", "prepped_soup", "prepped_grill"],
		"analysis prep rows are keyed by mise item"
	)
	var soup_row: Dictionary = report.prep.prepped_soup
	expect(
		(
			soup_row.planned == 2
			and soup_row.labor_units == 2
			and soup_row.menu_count == 1
			and soup_row.planned == soup_row.used + soup_row.remaining
		),
		"a prep row carries planned, used, remaining, labor and menu count"
	)
	for recommendation: Dictionary in report.recommendations:
		if recommendation.category == "prep":
			expect(
				started.definitions.ingredient_for(recommendation.target_id) != null,
				"prep recommendations target a mise item"
			)
...
func _three_item_soup_fixture() -> Resource:
	...
	soup.set(
		"mise_ids", PackedStringArray(["prepped_soup_grain", "prepped_soup_vegetable", "prepped_soup_vegetable_2"])
	)
...
func _test_mise_set_consumption() -> void:
	var data := _split_soup_fixture()
	expect(
		_ingredient(data, "prepped_soup_grain").get("inputs") == _typed_inputs({"grain": 1}),
		"the split fixture keeps its typed inputs"
	)
	...
	expect(
		(
			view.orders[0].uses_prepared
			and view.inventory.prepped_soup_grain == 0
			and view.inventory.prepped_soup_vegetable == 0
			and view.orders[0].consumed_cost == 350
		),
		"a complete mise set is consumed item by item at its combined cost"
	)
	...
	expect(
		depleted_ids == ["prepped_soup_grain", "prepped_soup_vegetable"],
		"each mise item that reaches zero emits its own depletion event"
	)
	var saved: Dictionary = full.export_state()
	var restored := ServiceSim.restore(
		data, saved, {"prep_quantities": {"prepped_soup_grain": 1, "prepped_soup_vegetable": 1}}
	)
	expect(
		restored.accepted and restored.simulation.state_hash() == full.state_hash(),
	...
	var contradictory_restore := ServiceSim.restore(
		data, contradictory, {"prep_quantities": {"prepped_soup_grain": 1, "prepped_soup_vegetable": 1}}
	)
	expect(
		not contradictory_restore.accepted and contradictory_restore.reason == "invalid_consumption",
...
func _test_partial_prep() -> void:
	...
	_run_until(
		partial,
		40,
		func(view: Dictionary) -> bool: return view.orders.size() > 0 and view.orders[0].ingredients_reserved
	)
	var view: Dictionary = partial.snapshot()
	expect(
		(
			view.orders[0].reserved_inputs == {"prepped_soup_grain": 1, "vegetable": 2}
			and view.orders[0].missing_mise_ids == ["prepped_soup_vegetable"]
			and not view.orders[0].uses_prepared
		),
	...
	expect(
		(
			view.orders[0].raw_consumed
			and view.inventory.prepped_soup_grain == 0
			and view.inventory.vegetable == 20
			and view.inventory.grain == 7
			and view.orders[0].consumed_cost == 350
		),
	...
	_run_until(
		partial,
		200,
		func(current: Dictionary) -> bool:
			return current.orders[0].state == "working" and current.orders[0].phase_id == "prep"
	)
	view = partial.snapshot()
	expect(
		(
			view.orders[0].phase_id == "prep"
			and view.tasks.size() == 1
	...
	expect(
		working_restore.accepted and working_restore.simulation.state_hash() == partial.state_hash(),
	...
	expect(
		not ServiceSim.restore(data, reordered, options).accepted,
	...
	expect(
		not ServiceSim.restore(data, unknown, options).accepted,
	...
	expect(
		not understated_restore.accepted and understated_restore.reason == "invalid_consumption",
	...
	expect(
		not overstated_restore.accepted and overstated_restore.reason == "invalid_inventory",
	...
	_run_until(
		...
		func(current: Dictionary) -> bool:
			return (
				current.orders.size() > 0
				and current.orders[0].state == "working"
				and current.orders[0].phase_id == "prep"
			)
	)
	view = two_missing.snapshot()
	expect(
		(
			view.orders[0].missing_mise_ids == ["prepped_soup_vegetable", "prepped_soup_vegetable_2"]
			and view.tasks[0].completion_tick - view.tasks[0].started_tick == 47
		),
		"two missing items of three round 70 × 2/3 up to 47 ticks"
	)
	var one_missing := ServiceSim.new(
		three, null, {"prep_quantities": {"prepped_soup_grain": 1, "prepped_soup_vegetable": 1}}
	)
	_run_until(
		one_missing,
		200,
	...
	expect(
		(
			view.orders[0].missing_mise_ids == ["prepped_soup_vegetable_2"]
	...
	_run_until(
		...
		func(current: Dictionary) -> bool:
			return (
				current.orders.size() == 2
				and current.orders[0].input_consumed
				and current.orders[1].wait_reason == "missing_ingredients"
			)
	)
	view = starved.snapshot()
	expect(
		(
			view.orders[1].wait_reason == "missing_ingredients"
			and not view.orders[1].ingredients_reserved
			and view.orders[1].reserved_inputs.is_empty()
			and view.reserved.values().all(func(value: int) -> bool: return value == 0)
		),
...
func _test_campaign_mise_content() -> void:
	...
	expect(
		final_service.get("ingredients").size() == 10, "the final service lists four raw ingredients and six mise items"
	)
	for mise_id: String in CAMPAIGN_MISE:
		...
		expect(
			(
				item != null
				and item.get("display_name") == expected.name
				and item.get("inputs") == _typed_inputs(expected.inputs)
				and item.get("labor_units") == expected.labor
				and item.get("unit_cost") == expected.cost
			),
			"campaign mise item matches the approved table: " + mise_id
		)
		expect(
			item != null and final_service.call("menu_count_for", mise_id) == expected.menus,
	...
	for id: String in CAMPAIGN_NAMES:
		...
		expect(
			definition != null and definition.get("display_name") == CAMPAIGN_NAMES[id],
			"campaign display name is a real food: " + id
		)
	expect(
		(
			final_service.call("recipe_for", "grain_grill").get("mise_ids")
			== PackedStringArray(["prepped_grain", "prepped_mushroom"])
		),
		"the rice stir-fry uses grain and mushroom mise, not protein"
	)
	TranslationServer.set_locale("en")
	expect(
		TranslationServer.translate("재운 연어") == "Marinated salmon", "the English translation covers the new mise names"
	)
...
func _test_shared_mise_stock() -> void:
	...
	expect(
		_command(short_plan, "set_purchase", "vegetable", 3, 1).accepted, "lunch_prep accepts a vegetable purchase of 3"
	)
	expect(_command(short_plan, "set_purchase", "grain", 1, 2).accepted, "lunch_prep accepts a grain purchase of 1")
	expect(
		_command(short_plan, "set_prep", "prepped_vegetable", 1, 3).accepted,
		"one prepped_vegetable fits the labor budget"
	)
	...
	expect(
		not short_snapshot.can_start and "menu_missing_ingredients" in short_snapshot.errors,
	...
	expect(
		_command(full_plan, "set_purchase", "vegetable", 4, 1).accepted, "lunch_prep accepts a vegetable purchase of 4"
	)
	expect(_command(full_plan, "set_purchase", "grain", 2, 2).accepted, "lunch_prep accepts a grain purchase of 2")
	expect(
		_command(full_plan, "set_prep", "prepped_vegetable", 2, 3).accepted,
		"two prepped_vegetable fit the labor budget"
	)
	...
	expect(
		full_snapshot.can_start and full_snapshot.errors.is_empty(),
...
func _test_shared_raw_stock() -> void:
	...
	expect(
		_command(plan, "set_prep", "prepped_vegetable", 1, 3).accepted, "one prepped_vegetable fits the labor budget"
	)
	...
	expect(
		snapshot.inventory.vegetable == 1 and snapshot.inventory.grain == 0,
		"the mixed-stock plan leaves one raw vegetable and no raw grain"
	)
	expect(
		snapshot.can_start and snapshot.errors.is_empty(),
...
func _test_raw_attribution() -> void:
	...
	expect(
		consumed.soup + consumed.grain_salad >= 4 and grain_row.used == 4 and grain_row.remaining == 0,
		"mixed consumption spends every prepared grain item on the first grain orders"
	)
	expect(
		grain_row.raw_orders == consumed.soup + consumed.grain_salad - 4,
		"raw orders are attributed to the grain item only once its stock is gone"
	)
	expect(
		vegetable_row.raw_orders == consumed.salad + consumed.grain_salad and base_row.raw_orders == consumed.soup,
	...
	expect(
		(
			not prep_advice.is_empty()
			and not (prep_advice.action == "reduce_prep" and prep_advice.target_id == "prepped_grain")
		),
