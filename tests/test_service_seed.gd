...
func _test_begin_rolls_back_when_save_fails(tree: SceneTree) -> void:
	...
	if service != null:
		...
		await tree.process_frame
		expect(
			service.get("definitions").service_seed == 0,
			"the retried first start still uses seed 0 because the draw was rolled back"
		)
	var document: Variant = JSON.parse_string(FileAccess.get_file_as_string(file_path))
	expect(
		document is Dictionary and document.attempts == {"first_shift": 1.0},
		"the successful start persists the attempt count once"
	)
	screen.queue_free()
	await tree.process_frame
...
func _test_campaign_screen(tree: SceneTree) -> void:
	...
	screen.get("begin_button").pressed.emit()
	expect(
		screen.get("active_service") == null and screen.get("replace_dialog").visible,
	...
	document = JSON.parse_string(FileAccess.get_file_as_string(file_path))
	expect(
		document.active_session.service_seed == float(attempt_one_seed),
		"the checkpoint stores attempt 1's service seed"
	)
	expect(
		document.attempts == {"first_shift": 2.0},
		"the deferred attempts save lands once attempt 1's checkpoint is written"
	)
	screen.queue_free()
	await tree.process_frame
...
func _test_replace_defers_attempts_save(tree: SceneTree) -> void:
	...
	expect(
		(
			pre_replace_document.attempts == {"first_shift": 1.0}
			and not pre_replace_document.active_session.simulation.closed
		),
	...
	expect(
		screen.get("replace_dialog").visible and screen.get("active_service") == null,
	...
	service.set_process(false)
	expect(
		service.get("definitions").service_seed == ScheduleGenerator.service_seed_for("first_shift", 1),
	...
	expect(
		(
			FileAccess.get_file_as_bytes(file_path) != pre_replace_bytes
			and confirmed_document.attempts == {"first_shift": 2.0}
		),
		"confirming replacement persists attempt count 2 immediately so a quit before Start cannot redraw the same seed"
	)
	expect(
		confirmed_document.active_session == pre_replace_document.active_session,
		"confirming replacement keeps the open checkpoint on disk until Start replaces it"
	)
	service.get("start_button").pressed.emit()
	var document: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(file_path))
	expect(document.attempts == {"first_shift": 2.0}, "the replacement's preparation checkpoint keeps attempt count 2")
	expect(
		document.active_session.service_seed == float(ScheduleGenerator.service_seed_for("first_shift", 1)),
...
func _test_scenario_fields(campaign: Resource) -> void:
	for scenario: Resource in campaign.scenarios:
		if scenario.id in FIXED_FORECAST:
			...
			expect(
				not scenario.forecast_slack.is_empty(),
				"every pressure service varies from the second attempt: " + scenario.id
			)
			var total: int = 0
			for recipe_id: String in scenario.menu_ids:
				...
				total += slack
				expect(
					slack >= 0 and slack <= maxi(1, scenario.baseline_counts()[recipe_id] / 5),
			...
			for attempt: int in [1, 2, 3, 4, 5]:
				expect(
					(
						ScheduleGenerator.recipe_ids(scenario, ScheduleGenerator.service_seed_for(scenario.id, attempt))
						!= scenario.order_recipe_ids
					),
	...
	expect(
		counts.get("grill") == 8 and counts.get("soup") == 4 and counts.get("salad") == 8,
		"baseline counts come from the authored order"
	)
	...
	expect(
		ranges.grill == {"baseline": 8, "min": 6, "max": 10} and ranges.soup == {"baseline": 4, "min": 3, "max": 5},
		"slack widens the range around the baseline"
	)
	...
	expect(
		seeded.service_seed == 7 and hot_queue.service_seed == 0,
		"with_service_seed returns a seeded copy and leaves the source untouched"
	)
	expect(
		seeded.id == hot_queue.id and seeded.order_recipe_ids == hot_queue.order_recipe_ids,
		"the seeded copy keeps the authored content"
	)
	expect(
		(
			slacked.maximum_profit(20)
			== (
				fixed.maximum_profit(20)
				+ 2 * _margin(hot_queue, "grill")
				+ _margin(hot_queue, "soup")
				- 3 * _margin(hot_queue, "salad")
			)
		),
		"maximum profit uses the forecast upper bounds"
	)
...
func _test_session_seed(campaign: Resource) -> void:
	...
	expect(
		started.definitions.service_seed == 104076537,
		"the started definition carries the service seed through duplicate()"
	)
	...
	expect(
		session.size() == 6 and session.service_seed == 104076537, "capture stores the service seed as the sixth field"
	)
	...
	expect(
		restored.definitions.order_schedule()[0].recipe_id == seeded.order_schedule()[0].recipe_id,
		"restore rebuilds the seeded schedule"
	)
	...
	expect(
		modified.scenario_for("hot_queue") == slacked and campaign.scenario_for("hot_queue") == hot_queue,
	...
	var drawn_started := drawn_plan.apply_command(
	...
	expect(
		ServiceSession.restore(modified, drawn_session, records).accepted,
		"a closed session restores under the seed that produced it"
	)
	...
	expect(
		not rejected.accepted and rejected.reason == "invalid_schedule",
		"the same orders cannot restore under a seed whose draw differs"
	)
...
func _test_attempts(campaign: Resource) -> void:
	...
	expect(
		first.accepted and first.service_seed == 0 and first.attempt_index == 0,
		"the first start of a service uses seed 0"
	)
	var second := progress.next_service_seed("first_shift")
	expect(
		(
			second.accepted
			and second.service_seed == ScheduleGenerator.service_seed_for("first_shift", 1)
			and second.attempt_index == 1
		),
		"the second start draws attempt 1"
	)
	...
	expect(
		progress.revert_service_seed("first_shift", 1) and progress.snapshot().attempts == {"first_shift": 1},
		"reverting the last draw restores the previous count"
	)
	expect(
		progress.revert_service_seed("first_shift", 0) and progress.snapshot().attempts == {},
		"reverting the first draw removes the entry"
	)
	expect(
		progress.next_service_seed("first_shift").service_seed == 0,
		"the next draw after a full revert is attempt 0 again"
	)
	...
	expect(
		not CampaignProgress.validate_attempts(campaign, {"missing": 1}).is_empty(),
		"attempts for an unknown service fail"
	)
	expect(not CampaignProgress.validate_attempts(campaign, {"first_shift": -1}).is_empty(), "negative attempts fail")
	expect(
		not CampaignProgress.validate_attempts(campaign, {"first_shift": 1.5}).is_empty(), "non-integer attempts fail"
	)
...
func _test_store_schema(campaign: Resource) -> void:
	...
	var document: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(file_path))
	expect(
		document.size() == 6 and int(document.schema_version) == 4 and document.attempts == {"first_shift": 3.0},
		"new writes use schema 4 with an attempts key"
	)
	...
	expect(
		CampaignStore.new(campaign, file_path).load_records().attempts == {"first_shift": 3},
		"a null attempts argument preserves the primary attempts"
	)
	...
	document = JSON.parse_string(FileAccess.get_file_as_string(file_path))
	expect(
		int(document.schema_version) == 4 and document.attempts == {"first_shift": 1.0},
		"the upgraded document carries schema 4 and attempts"
	)
	var corrupt := {
		"schema_version": 4,
		"content_version": 4,
	...
	expect(
		...
		"attempts for an unknown service are rejected as corrupt"
	)
	var missing_attempts := {
		"schema_version": 4, "content_version": 4, "sim_version": 1, "records": {}, "active_session": null
	}
	file = FileAccess.open(file_path, FileAccess.WRITE)
	...
	expect(
		not loaded.accepted and loaded.reason == "corrupt_records",
		"a schema 4 document without an attempts key is rejected as corrupt"
	)
	var non_dictionary_attempts := {
		"schema_version": 4,
		"content_version": 4,
		"sim_version": 1,
		"records": {},
	...
	expect(
		not loaded.accepted and loaded.reason == "corrupt_records",
		"a schema 4 document whose attempts is not a Dictionary is rejected as corrupt"
	)
	var session_plan := PreparationPlan.new(campaign.scenario_for("first_shift"))
	var session_started := session_plan.apply_command(
		{"kind": "start", "target_id": "", "value": null, "apply_tick": 0, "sequence": 1}
	)
	...
	loaded = CampaignStore.new(campaign, file_path).load_records()
	expect(
		loaded.accepted and loaded.active_session is Dictionary and loaded.active_session.service_seed == 0,
		"a schema 3 document still restores a five-field session, normalized to seed 0"
	)
	...
	expect(
		(
			int(staged_backup.schema_version) == 4
			and staged_backup.active_session is Dictionary
			and staged_backup.active_session.service_seed == 0
		),
		"the staged backup re-encodes the legacy session with service_seed 0"
	)
	expect(upgrading_store.clear_active_session().accepted, "clearing a normalized legacy session succeeds")
	expect(
		CampaignStore.new(campaign, file_path).load_records().active_session == null,
		"the legacy session is cleared on disk"
	)
	var truncated_session_document := {
		"schema_version": 4,
		"content_version": 4,
		"sim_version": 1,
		"records": {},
		"active_session": five_field_session,
		"attempts": {}
	}
	file = FileAccess.open(file_path, FileAccess.WRITE)
	...
	expect(
		not loaded.accepted and loaded.reason == "corrupt_records",
		"a schema 4 document whose session lacks service_seed is rejected as corrupt"
	)
	...
	expect(
		loaded.accepted and loaded.active_session is Dictionary and loaded.active_session.service_seed == 0,
		"a schema 4 document with a six-field session restores"
	)
	for owned_file: String in DirAccess.get_files_at(directory):
		DirAccess.remove_absolute(directory + "/" + owned_file)
	DirAccess.remove_absolute(directory)
