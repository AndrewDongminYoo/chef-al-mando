	...
	for scenario_id: String in ["hot_queue", "long_route"]:
		...
		for layout: String in ["original", "clustered"]:
			for duties: String in ["all", "dedicated"]:
				...
				var expected_prep: bool = result.selection.prep_quantities.prepped_grill == (1 if scenario_id == "hot_queue" else 4) \
					and (scenario_id != "hot_queue" or result.selection.prep_quantities.prepped_salad == 3)
