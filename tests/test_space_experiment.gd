...
func run(_tree: SceneTree) -> void:
	...
	for scenario_id: String in ["hot_queue", "long_route"]:
		...
		expect(
			scenario != baseline and scenario.space_rules and baseline.space_rules,
			"spatial experiments clone the campaign space rules"
		)
		expect(
			scenario.order_schedule() == baseline.order_schedule() and scenario.purchases == baseline.purchases,
			"an experiment preserves orders and purchases: " + scenario_id
		)
		for index: int in scenario.stations.size():
			...
			expect(
				station.fixed == expected_fixed,
				"the experiment preserves each campaign fixture anchor: " + scenario_id + "/" + station.role
			)
			expect(
				(
					scenario.stations[index].tile == baseline.stations[index].tile
					and scenario.stations[index].work_position == baseline.stations[index].work_position
				),
				"fixed experiment anchors use their original positions"
			)
		...
		for layout: String in ["original", "clustered"]:
			for duties: String in ["all", "dedicated"]:
				...
				expect(
					policy.priorities == {},
					(
						"policy comparisons keep runtime priority commands empty: "
						+ scenario_id
						+ "/"
						+ layout
						+ "/"
				...
				var expected_prep: bool = (
					result.selection.prep_quantities.marinated_protein == (1 if scenario_id == "hot_queue" else 4)
					and (scenario_id != "hot_queue" or result.selection.prep_quantities.prepped_vegetable == 3)
				)
				expect(
					result.selection.purchases == baseline.purchases and expected_prep,
				...
				if layout == "clustered" and duties == "all":
					...
					expect(
						repeat.accepted and repeat.hash == result.hash and fast.accepted and fast.hash == result.hash,
						"repeated spatial policies and 1x/4x preserve the same final state"
					)
		if results.size() == 4:
			expect(
				(
					results["original/all"].snapshot.metrics.orders.moving
					!= results["clustered/all"].snapshot.metrics.orders.moving
				),
				"changing the legal layout changes actual movement time"
			)
			var common: Dictionary = results["original/all"].selection
			for result: Dictionary in results.values():
				expect(
					(
						result.selection.purchases == common.purchases
						and result.selection.prep_quantities == common.prep_quantities
						and result.selection.menu_priorities == common.menu_priorities
					),
					"policy comparisons isolate layout and duty changes"
				)
		...
		expect(
			not rejected.accepted and rejected.reason.contains("fixed_station"),
			"an old policy that moves the pass is classified as invalid instead of a service loss"
		)
...
func _test_session_rules(experiment: GDScript) -> void:
	...
	for variant: String in ["valid", "moved_pass", "shared_work"]:
		...
		for tick: int in 11:
			simulation.step()
		var session: Dictionary = JSON.parse_string(
			JSON.stringify(ServiceSession.capture("hot_queue", started.selection, simulation))
		)
		var legacy_restore := ServiceSession.restore(legacy_campaign, session, records)
		expect(
			legacy_restore.accepted and legacy_restore.simulation.state_hash() == simulation.state_hash(),
			"a legacy kitchen restores its original session"
		)
		var experiment_restore := ServiceSession.restore(baseline, session, records)
		if variant == "valid":
			expect(
				experiment_restore.accepted and experiment_restore.simulation.state_hash() == simulation.state_hash(),
				"a legal campaign session restores its exact running state"
			)
		else:
			expect(
				not experiment_restore.accepted and experiment_restore.reason == "invalid_preparation",
				"campaign session restoration applies the current spatial rules: " + variant
			)
