...
func run(_tree: SceneTree) -> void:
	...
	expect(
		...
		"content limits rows must cover versions 1 to %d in order, found %s; %s" % [live_version, versions, DUMP_HINT]
	)
	var latest: Dictionary = rows[-1] if not rows.is_empty() else {}
	expect(
		latest.get("content_version") == live_version,
		(
			"the last content limits row is content %s but the live content version is %d; %s"
			% [latest.get("content_version"), live_version, DUMP_HINT]
		)
	)
	expect(
		latest == live,
		(
			"the live content differs from the last content limits row; bump the content version if a save could notice, then %s"
			% DUMP_HINT
		)
	)
	# Dictionary == ignores key order, and the key order is the campaign order validate_records checks.
	expect(
		latest.get("scenarios", {}).keys() == live.scenarios.keys(),
		(
			# gdlint: ignore=max-line-length
	...
	for index: int in rows.size():
		...
		expect(
			problems.is_empty(),
			"every record content %d accepted stays valid under the live content" % row.content_version
		)
...
static func compatibility_problems(
	row: Dictionary,
	campaign: Resource,
	later_rows: Array = [],
	targets: Dictionary = CampaignProgress.LEGACY_COMPLETION_TARGETS
) -> Array[String]:
	...
	for scenario_id: String in row.scenarios:
			...
			(
				...
				. append(
					(
						prefix
						+ (
							"order_count fell from %d to %d; a best_served above it is rejected and no legacy table can cover that"
		...
		for served: int in mini(old.order_count, scenario.order_count) + 1:
			var bound: int = maxi(
				scenario.maximum_profit(served), CampaignProgress.legacy_maximum_profit(scenario_id, served)
			)
			...
			(
				...
				. append(
					(
						prefix
						+ (
							"maximum_profit(%d) was %d but the live bound is %d (%d served counts affected); add a LEGACY_PROFIT_CAPS entry"
							% [
								first,
								old.maximum_profit[first],
								maxi(
									scenario.maximum_profit(first),
									CampaignProgress.legacy_maximum_profit(scenario_id, first)
								),
								uncovered.size()
							]
						)
			...
			(
				...
				. append(
					(
						prefix
						+ (
							"starting_budget fell from %d to %d; a best_profit down to -%d is rejected and no legacy table can cover that"
							% [old.starting_budget, scenario.starting_budget, old.starting_budget]
						)
					)
				)
			)
		var pair_changed: bool = (
			old.minimum_served != scenario.minimum_served or old.minimum_profit != scenario.minimum_profit
		)
		for later: Dictionary in later_rows:
			var next: Dictionary = later.scenarios.get(scenario_id, {})
			if (
				not next.is_empty()
				and (next.minimum_served != old.minimum_served or next.minimum_profit != old.minimum_profit)
			):
				pair_changed = true
		if pair_changed:
			var covered := false
			for pair: Dictionary in targets.get(scenario_id, []):
				if (
					pair.minimum_served == old.minimum_served
					and pair.minimum_profit == old.minimum_profit
					and pair.since_content <= version
				):
					covered = true
			if not covered:
				(
					...
					. append(
						(
							prefix
							+ (
								"target pair %d served / %d profit needs a LEGACY_COMPLETION_TARGETS entry with since_content at most %d"
		...
		if not missing.is_empty():
			(
				...
				. append(
					(
						# gdlint: ignore=max-line-length
						"content %d %s: the live campaign order puts %s before it, so its record in a save that completed only the services content %d put before it is rejected; no legacy table can cover that, so keep the old order and add new services after it"
...
func _expect_detects_each_lockout(campaign: Resource, live: Dictionary) -> void:
	for cause: String in ["order_count", "maximum_profit", "starting_budget", "target pair"]:
		...
		expect(
			problems.size() == 1 and problems[0].begins_with("content 1 first_shift: ") and cause in problems[0],
	...
	expect(
		early_problems.size() == 1 and "since_content at most 2" in early_problems[0],
	...
	expect(
		(
			restored_problems.size() == 1
			and restored_problems[0].begins_with("content 1 first_shift: ")
			and "target pair" in restored_problems[0]
		),
		(
			"the content limits check reports a restored target pair that no legacy entry covers, got %s"
			% [restored_problems]
		)
	)
	expect(
		compatibility_problems(restored, campaign, [replaced]).is_empty(),
		"the legacy first_shift entry covers the restored target pair"
	)
	expect(
		compatibility_problems(restored, campaign, [], without_first_shift).is_empty(),
	...
	expect(
		(
			swapped_problems.size() == 1
			and swapped_problems[0].begins_with('content 1 hot_queue: the live campaign order puts ["lunch_prep"]')
		),
	...
	expect(
		(
			inserted_problems.size() == inserted.scenarios.size() - 1
			and all_name_lunch_prep
			and inserted_problems[0].begins_with("content 1 hot_queue: ")
		),
		(
			"the content limits check reports a service the live campaign inserted before older ones, got %s"
			% [inserted_problems]
		)
	)
	...
	expect(
		(
			renamed_problems.size() == 1
			and renamed_problems[0].begins_with(
				"content %d retired_service: the service no longer exists" % live.content_version
			)
		),
