...
func command(plan: PreparationPlan, kind: String, station_id: String, value: Variant, sequence: int = 1) -> Dictionary:
	return plan.apply_command(
		{"kind": kind, "target_id": station_id, "value": value, "apply_tick": 0, "sequence": sequence}
	)
...
func _test_shared_work_position() -> void:
	...
	expect(
		not result.accepted and result.reason == "work_position_overlap",
		"moving two work positions onto one tile is rejected"
	)
	expect(
		JSON.stringify(plan.snapshot(), "", true) == before,
		"an overlap rejection preserves choices and command sequence"
	)
	data.stations[1].tile = Vector2i(3, 2)
	data.stations[1].work_position = Vector2i(2, 2)
	expect(
		data.placement_error() == "work_position_overlap",
		"initial content cannot share a work position under space rules"
	)
...
func _test_fixed_station() -> void:
	...
	for choice: Array in [["move_station", "left"], ["rotate_station", null]]:
		...
		expect(
			JSON.stringify(plan.snapshot(), "", true) == before,
			"fixed-station rejection preserves the complete preparation"
		)
...
func _test_cold_hot_clearance() -> void:
	...
	expect(
		command(plan, "move_station", "hot_01", "left").accepted,
		"one tile between cold and hot station bodies is allowed"
	)
	var before := JSON.stringify(plan.snapshot(), "", true)
...
func _test_restore_selection() -> void:
	...
	expect(
		not restored.snapshot().can_start and "fixed_station" in restored.snapshot().errors,
		"restoring preparation cannot bypass a fixed station"
	)
	...
	expect(
		not restored.snapshot().can_start and "work_position_overlap" in restored.snapshot().errors,
		"restoring preparation cannot bypass independent work positions"
	)
...
func _test_preview() -> void:
	...
	expect(
		options.get("left") == "fixed_station" and options.get("rotate") == "fixed_station",
		"fixed movement and rotation are explained before tapping"
	)
	expect(
		command(plan, "move_station", "hot_01", "left").accepted,
		"the clearance preview fixture starts one move before the limit"
	)
	state = plan.snapshot()
	options = state.stations[2].get("placement_options", {})
	expect(
		options.get("left") == "cold_hot_adjacent" and options.get("right") == "",
		"placement preview distinguishes an invalid direction from a valid one"
	)
