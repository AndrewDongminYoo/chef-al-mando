...
func run(_tree: SceneTree) -> void:
	...
	expect(
		original.snapshot().tasks.size() == 1 and original.snapshot().employees[0].progress in [1, 2, 3, 4],
	...
	if not original.has_method("export_state") or not method_names.has("restore"):
		...
		return
	var restored: Dictionary = service_sim_script.call(
		"restore", started.definitions, original.call("export_state"), started.options
	)
	...
	if restored.get("accepted") == true:
		expect(
			...
			"restoration preserves the immediate simulation hash"
		)
		expect(
			not original.events().is_empty() and restored.simulation.events().is_empty(),
...
func _test_inventory_corruption(
	data: Resource, preparation: Dictionary, original: RefCounted, service_sim_script: GDScript
) -> void:
	...
	expect(
		...
		"restore rejects inventory that cannot follow from preparation and consumption"
	)
	expect(original.call("state_hash") == before, "failed restore leaves the existing simulation unchanged")


func _test_reserved_input_json_type(
	data: Resource, preparation: Dictionary, original: RefCounted, service_sim_script: GDScript
) -> void:
	...
	expect(
		(
			state is Dictionary
			and state.orders[0].state == "moving"
			and state.orders[0].ingredients_reserved
			and state.orders[0].reserved_inputs.vegetable is int
		),
	...
	expect(
		malformed_reservation is Dictionary and malformed_reservation.vegetable is String,
	...
	expect(
		not result.accepted and result.reason == "invalid_reservation",
		"direct restore rejects a JSON string reservation with a structured reason"
	)
	expect(original.state_hash() == source_hash, "the rejected JSON reservation leaves the source simulation unchanged")
...
func _test_export_copy(original: RefCounted) -> void:
	...
	expect(
		(
			exported.get("prng_state", "missing") == null
			and not exported.has("accounting")
			and not exported.has("events")
			and not exported.has("errors")
		),
...
func _test_corruption_table(
	data: Resource, preparation: Dictionary, moving: RefCounted, service_sim_script: GDScript
) -> void:
...
func _test_identity_and_type_corruption(data: Resource, preparation: Dictionary, service_sim_script: GDScript) -> void:
	...
	expect(
		duplicate_result is Dictionary and not duplicate_result.get("accepted", false),
	...
	expect(
		terminal_result is Dictionary and not terminal_result.get("accepted", false),
...
func _test_task_back_references(data: Resource, preparation: Dictionary, service_sim_script: GDScript) -> void:
	...
	expect(
		(
			not worker.is_empty()
			and worker.progress == 0
			and worker.tile == worker.next_tile
			and worker.order_id == task.order_id
		),
		"the back-reference fixture is valid active work"
	)
	worker.order_id = ""
	var missing_back_reference: Dictionary = service_sim_script.call("restore", data, working_state, preparation)
	expect(
		not missing_back_reference.get("accepted", false),
		"restore rejects a working task whose employee lost the order back-reference"
	)
	var idle: RefCounted = service_sim_script.new(data, null, preparation)
	var orphan_state: Dictionary = idle.export_state()
	expect(
		orphan_state.orders.is_empty() and orphan_state.tasks.is_empty(),
		"the orphan fixture starts at an empty tick boundary"
	)
...
func _test_between_process_waiting(service_sim_script: GDScript) -> void:
	for prepared: bool in [false, true]:
		...
		expect(
			(
				waiting_order.state == "waiting"
				and waiting_order.phase_index > 0
				and waiting_order.has_result
				and not waiting_order.carrying
				and view.tasks.is_empty()
			),
		...
		if prepared:
			expect(
				(
					waiting_order.uses_prepared
					and waiting_order.intermediate_ready
					and not waiting_order.raw_consumed
					and not waiting_order.intermediate_consumed
				),
			...
			expect(
				(
					waiting_order.raw_consumed
					and not waiting_order.uses_prepared
					and not waiting_order.intermediate_ready
					and not waiting_order.intermediate_consumed
				),
				"the raw fixture preserves its pickup result before preparation"
			)
		var restored: Dictionary = service_sim_script.call(
			"restore", fixture.definitions, simulation.export_state(), fixture.options
		)
		expect(
			restored.get("accepted", false) and restored.simulation.state_hash() == simulation.state_hash(),
		...
		var invalid_intermediate_result: Dictionary = service_sim_script.call(
			"restore", fixture.definitions, invalid_intermediate, fixture.options
		)
		expect(
			not invalid_intermediate_result.get("accepted", false),
		...
		for sequence: int in [3, 4]:
			...
			expect(
				simulation.enqueue_command(command).accepted and restored.simulation.enqueue_command(command).accepted,
		...
		expect(
			(
				moving_view.orders[0].state == "moving"
				and moving_view.orders[0].phase_index > 0
				and moving_view.orders[0].has_result
				and moving_view.tasks[0].collection_index >= 0
			),
		...
		var skipped_collection_result: Dictionary = service_sim_script.call(
			"restore", fixture.definitions, skipped_collection, fixture.options
		)
		expect(
			not skipped_collection_result.get("accepted", false),
...
func _between_process_fixture(service_sim_script: GDScript, prepared: bool) -> Dictionary:
	...
	var started := plan.apply_command(
		{"kind": "start", "target_id": "", "value": null, "apply_tick": 0, "sequence": preparation_sequence + 1}
	)
...
func _test_canonical_task_paths(service_sim_script: GDScript) -> void:
	...
	expect(direct_state.tasks[0].collection_index == -1, "the direct-path fixture moves without a collection leg")
	...
	var before_routes := GridRoutes.new(
		before_collection.definitions.grid_size, before_collection.definitions.blocked_tiles()
	)
	expect(
		(
			before_task.collection_index > 0
			and before_task.path_index < before_task.collection_index
	...
	expect(
		(
			before_task.path[0] == [3, 4]
			and before_task.path[before_task.collection_index] == [2, 3]
			and before_task.path[-1] == [5, 3]
			and before_routes.is_walkable(Vector2i(2, 4))
	...
	expect(_path_has_overlap(before_task.path), "the real two-leg collection path legitimately revisits a route tile")
	...
	expect(
		(
			after_task.collection_index > 0
			and after_task.path_index >= after_task.collection_index
...
func _collection_path_fixture(service_sim_script: GDScript, after_collection: bool) -> Dictionary:
	...
	expect(
		not final_state.tasks.is_empty() and final_state.tasks[0].employee_id == "employee_02",
...
func _assert_detour_rejected(fixture: Dictionary, service_sim_script: GDScript, label: String) -> void:
	...
	expect(
		(
			not worker.is_empty()
			and worker.progress in [1, 2, 3, 4]
	...
	expect(
		not result.get("accepted", false) and result.get("reason", "") == "invalid_path",
		"restore rejects a connected noncanonical task detour: " + label
	)
	expect(simulation.state_hash() == source_hash, "the rejected task detour preserves the source simulation: " + label)
	...
	for progress: int in [1, 4]:
		...
		expect(
			(
				...
				and corrupted.orders[0].metrics.no_route == 0
		...
		expect(
			not result.get("accepted", false) and result.get("reason", "") == "invalid_employee",
			"restore rejects in-range movement progress that changes elapsed travel"
		)
		expect(
			fixture.simulation.state_hash() == source_hash,
...
func _test_result_position_corruption(service_sim_script: GDScript) -> void:
	for prepared: bool in [false, true]:
		...
		expect(
			(
				state.orders[0].state == "waiting"
				and state.orders[0].has_result
				and state.orders[0].metrics.no_route == 0
				and state.orders[0].result_position == [2, 3]
		...
		for invalid_tile: Array in [[3, 3], [5, 3]]:
			expect(
				routes.is_walkable(Vector2i(invalid_tile[0], invalid_tile[1])),
			...
			expect(
				not result.get("accepted", false) and result.get("reason", "") == "invalid_order",
	...
	for carrying: bool in [false, true]:
		for checkpoint: String in ["waiting", "retry"]:
			...
			if checkpoint == "retry":
				...
				expect(
					(
						retried.orders[0].state == "moving"
						and worker.get("progress", 0) == 1
...
func _partial_path_failure_fixture(service_sim_script: GDScript, carrying: bool) -> Dictionary:
	var fixture := (
		_collection_path_fixture(service_sim_script, true) if carrying else _prepared_service(service_sim_script)
	)
	if carrying:
	...
	expect(
		worker.get("progress", 0) == 1 and before.orders[0].carrying == carrying,
	...
	expect(
		(
			failed.orders[0].state == "waiting"
			and failed.orders[0].wait_reason == "no_route"
			and failed.orders[0].metrics.no_route > 0
			and failed.orders[0].metrics.moving % 5 == 1
	...
	if carrying:
		expect(
			(
				failed.orders[0].result_position == [3, 3]
				and failed.orders[0].has_result
...
func _test_active_state_relationships(service_sim_script: GDScript) -> void:
	...
	expect(
		(
			waiting_view.orders[0].state == "waiting"
			and waiting_view.orders[0].phase_index == 0
			and not waiting_view.orders[0].has_result
			and waiting_view.tasks.is_empty()
		),
		"the initial-waiting fixture has no process result"
	)
	var waiting_state: Dictionary = waiting.simulation.export_state()
	var waiting_restore: Dictionary = service_sim_script.call(
		"restore", waiting.definitions, waiting_state, waiting.options
	)
	...
	var fake_waiting_restore: Dictionary = service_sim_script.call(
		"restore", waiting.definitions, fake_waiting_result, waiting.options
	)
	expect(
		not fake_waiting_restore.get("accepted", false),
	...
	var premature_consumption_restore: Dictionary = service_sim_script.call(
		"restore", waiting.definitions, premature_consumption, waiting.options
	)
	expect(
		not premature_consumption_restore.get("accepted", false),
	...
	while moving.simulation.tick < 100:
		...
		if (
			not moving.simulation.snapshot().orders.is_empty()
	...
	expect(
		(
			moving_view.orders[0].state == "moving"
			and moving_view.orders[0].phase_index == 0
			and not moving_view.orders[0].has_result
			and moving_view.tasks[0].collection_index == -1
		),
		"the initial-moving fixture has no result-collection path"
	)
	var moving_state: Dictionary = moving.simulation.export_state()
	var moving_restore: Dictionary = service_sim_script.call(
		"restore", moving.definitions, moving_state, moving.options
	)
	...
	var fake_moving_restore: Dictionary = service_sim_script.call(
		"restore", moving.definitions, fake_moving_result, moving.options
	)
	expect(
		not fake_moving_restore.get("accepted", false),
				...
				. enqueue_command(
					{
						...
						"target_id": employee_id,
						"value": "all",
						"apply_tick": prepared.simulation.tick + 1,
						"sequence": sequence
					}
				)
				. accepted
	...
	expect(
		(
			cook_moving_view.orders[0].state == "moving"
			and cook_moving_view.orders[0].phase_id == "cook"
	...
	var cook_moving_restore: Dictionary = service_sim_script.call(
		"restore", prepared.definitions, cook_moving_state, prepared.options
	)
	...
	var erased_intermediate_restore: Dictionary = service_sim_script.call(
		"restore", prepared.definitions, cook_moving_state, prepared.options
	)
	expect(
		not erased_intermediate_restore.get("accepted", false),
	...
	expect(
		(
			cook_working_view.orders[0].state == "working"
			and cook_working_view.orders[0].phase_id == "cook"
	...
	var cook_working_restore: Dictionary = service_sim_script.call(
		"restore", prepared.definitions, cook_working_state, prepared.options
	)
	...
	var skipped_consumption_restore: Dictionary = service_sim_script.call(
		"restore", prepared.definitions, cook_working_state, prepared.options
	)
	expect(
		not skipped_consumption_restore.get("accepted", false),
...
func _test_terminal_relationships(service_sim_script: GDScript) -> void:
	...
	expect(
		(
			not late.simulation.closed
			and late.simulation.tick == 2999
			and preclose.tasks.size() == 1
			and preclose.orders[0].state in ["moving", "working"]
			and preclose.orders[0].raw_consumed
		),
	...
	var resumed: Dictionary = service_sim_script.call(
		"restore", late.definitions, late.simulation.export_state(), late.options
	)
	expect(
		resumed.get("accepted", false) and resumed.simulation.state_hash() == late.simulation.state_hash(),
		"restore accepts the reachable raw-consuming pre-close state"
	)
	late.simulation.step()
	resumed.simulation.step()
	expect(
		late.simulation.closed and resumed.simulation.state_hash() == late.simulation.state_hash(),
	...
	expect(
		(
			closed_order.state == "expired"
			and closed_order.terminal_reason == "service_closed"
			and closed_order.ended_tick == late.definitions.closing_tick
			and closed_state.tasks.is_empty()
		),
	...
	var served_at_close_result: Dictionary = service_sim_script.call(
		"restore", late.definitions, served_at_close, late.options
	)
	...
	var active_closed_result: Dictionary = service_sim_script.call(
		"restore", late.definitions, active_closed, late.options
	)
	...
	expect(
		not early_close_result.get("accepted", false),
	...
	expect(
		(
			deadline_order.state == "expired"
			and deadline_order.terminal_reason == "deadline"
	...
	var deadline_result: Dictionary = service_sim_script.call(
		"restore", deadline.definitions, deadline_state, deadline.options
	)
	...
	var actual_deadline_restore: Dictionary = service_sim_script.call(
		"restore", deadline.definitions, actual_deadline_state, deadline.options
	)
	...
	var active_after_deadline_restore: Dictionary = service_sim_script.call(
		"restore", deadline.definitions, active_after_deadline, deadline.options
	)
	expect(not active_after_deadline_restore.get("accepted", false), "restore rejects an active order at its deadline")
	...
	expect(
		(
			deadline_at_close_order.deadline_tick == deadline_at_close.definitions.closing_tick
			and deadline_at_close_order.state == "expired"
			and deadline_at_close_order.terminal_reason == "deadline"
	...
	var deadline_at_close_restore: Dictionary = service_sim_script.call(
		"restore", deadline_at_close.definitions, deadline_at_close_state, deadline_at_close.options
	)
	...
	var swapped_close_reason: Dictionary = service_sim_script.call(
		"restore", deadline_at_close.definitions, deadline_at_close_state, deadline_at_close.options
	)
	expect(
		not swapped_close_reason.get("accepted", false),
	...
	expect(
		served_order.state == "served" and served_order.ended_tick < served_order.deadline_tick,
	...
	var served_result: Dictionary = service_sim_script.call("restore", served.definitions, served_state, served.options)
...
func _single_order_fixture(service_sim_script: GDScript, arrival_tick: int) -> Dictionary:
	...
	return {
		"simulation": service_sim_script.new(started.definitions, null, started.options),
...
func _test_command_canonicalization(service_sim_script: GDScript) -> void:
	...
	var command := {
		...
		"target_id": "order_01",
		"value": {"ignored": true},
		"apply_tick": fixture.simulation.tick + 2,
		"sequence": 1,
		"extra": "discard"
	}
	var original_command := command.duplicate(true)
	expect(
		fixture.simulation.enqueue_command(command).accepted,
	...
	expect(
		(
			saved_command.size() == 5
			and saved_command.has("kind")
			and saved_command.has("target_id")
			and saved_command.has("value")
			and saved_command.has("apply_tick")
			and saved_command.has("sequence")
	...
	if not restored.get("accepted", false):
		return
	expect(
		restored.simulation.state_hash() == fixture.simulation.state_hash(),
...
func _test_work_timing_relationships(service_sim_script: GDScript) -> void:
	...
	while (
		initial.simulation.tick < 100
		and (
			initial.simulation.snapshot().orders.is_empty()
					...
					. enqueue_command(
						{
							...
							"target_id": employee_id,
							"value": "all",
							"apply_tick": later.simulation.tick + 1,
							"sequence": sequence
						}
					)
					. accepted
	...
	for entry: Dictionary in fixtures:
		...
		expect(
			(
				view.orders[0].state == "working"
				and task.started_tick >= view.orders[0].arrival_tick
		...
		var shifted_restore: Dictionary = service_sim_script.call(
			"restore", fixture.definitions, shifted, fixture.options
		)
		expect(
			not shifted_restore.get("accepted", false),
			"restore rejects working timestamps shifted before accumulated work: " + label
		)
		expect(
			simulation.state_hash() == before_hash,
...
func _test_tick_zero_employee_relationships(service_sim_script: GDScript) -> void:
	...
	expect(
		(
			state.tick == 0
			and state.orders.is_empty()
			and state.tasks.is_empty()
			and state.employees[0].duty == "cold"
			and state.employees[1].duty == "hot"
		),
	...
	var moved_result: Dictionary = service_sim_script.call(
		"restore", fixture.definitions, moved_employee, fixture.options
	)
	expect(
		not moved_result.get("accepted", false),
		"restore rejects a walkable tick-zero employee position that differs from preparation"
	)
	expect(simulation.state_hash() == source_hash, "the rejected tick-zero position preserves the source simulation")
	var changed_duty := state.duplicate(true)
	changed_duty.employees[0].duty = "hot"
	var duty_result: Dictionary = service_sim_script.call("restore", fixture.definitions, changed_duty, fixture.options)
	expect(
		not duty_result.get("accepted", false), "restore rejects a valid tick-zero duty that differs from preparation"
	)
	expect(simulation.state_hash() == source_hash, "the rejected tick-zero duty preserves the source simulation")
	if not restored.get("accepted", false):
		return
	var command := {"kind": "set_duty", "target_id": "employee_01", "value": "all", "apply_tick": 1, "sequence": 1}
	expect(
		simulation.enqueue_command(command).accepted and restored.simulation.enqueue_command(command).accepted,
	...
	expect(
		(
			later_state.tick > 0
			and later_state.employees[0].duty == "all"
			and not later_state.tasks.is_empty()
			and later_state.employees[0].progress in [1, 2, 3, 4]
		),
		"the tick-positive fixture contains an applied duty command and actual path progress"
	)
	var later_restore: Dictionary = service_sim_script.call(
		"restore", fixture.definitions, later_state, fixture.options
	)
	expect(
		later_restore.get("accepted", false) and later_restore.simulation.state_hash() == simulation.state_hash(),
...
func _test_active_duty_relationships(service_sim_script: GDScript) -> void:
	var moving := _single_order_fixture(service_sim_script, 10)
	while (
		moving.simulation.tick < 100
		and (moving.simulation.snapshot().orders.is_empty() or moving.simulation.snapshot().orders[0].state != "moving")
	):
		moving.simulation.step()
	var moving_state: Dictionary = moving.simulation.export_state()
	...
	expect(
		(
			moving_state.orders[0].state == "moving"
			and moving_employee_index >= 0
	...
	var off_duty_restore: Dictionary = service_sim_script.call(
		"restore", moving.definitions, off_duty_moving, moving.options
	)
	expect(
		not off_duty_restore.get("accepted", false),
		"restore rejects an active moving employee whose current duty is off"
	)
	expect(
		moving.simulation.state_hash() == moving_before_hash,
		"a rejected moving-duty relation preserves the source simulation"
	)

	var working := _single_order_fixture(service_sim_script, 10)
	while (
		working.simulation.tick < 100
		and (
			working.simulation.snapshot().orders.is_empty()
	...
	expect(
		(
			working_state.orders[0].state == "working"
			and working_state.orders[0].recipe_id == "salad"
			and working.definitions.recipe_for("salad").cook_role == "cold"
			and working_employee_index >= 0
	...
	var working_restore: Dictionary = service_sim_script.call(
		"restore", working.definitions, working_state, working.options
	)
	...
	var incompatible_restore: Dictionary = service_sim_script.call(
		"restore", working.definitions, incompatible_working, working.options
	)
	expect(
		not incompatible_restore.get("accepted", false),
		"restore rejects an active working employee with an incompatible cooking duty"
	)
	expect(
		working.simulation.state_hash() == working_before_hash,
	...
	expect(
		(
			...
			. enqueue_command(
				{
					"kind": "set_duty",
					"target_id": active_employee_id,
					"value": "off",
					"apply_tick": working.simulation.tick + 1,
					"sequence": 1
				}
			)
			. accepted
	...
	expect(
		(
			deferred_state.orders[0].state == "working"
	...
	var deferred_restore: Dictionary = service_sim_script.call(
		"restore", working.definitions, deferred_state, working.options
	)
	expect(
		(
			deferred_restore.get("accepted", false)
	...
	if deferred_restore.get("accepted", false):
		...
		expect(
			deferred_restore.simulation.state_hash() == working.simulation.state_hash(),
...
func _test_restore_checkpoints(service_sim_script: GDScript) -> void:
	...
	var duty_result: Dictionary = pending_duty.simulation.enqueue_command(
		{
			"kind": "set_duty",
			"target_id": "employee_01",
			"value": "off",
			"apply_tick": pending_duty.simulation.tick + 1,
			"sequence": 1
		}
	)
	pending_duty.simulation.step()
	expect(
		duty_result.accepted and pending_duty.simulation.snapshot().employees[0].pending_duty == "off",
	...
	queued.simulation.enqueue_command(
		{"kind": "set_priority", "target_id": "order_01", "value": 2, "apply_tick": apply_tick, "sequence": 1}
	)
	queued.simulation.enqueue_command(
		{"kind": "set_priority", "target_id": "order_01", "value": 0, "apply_tick": apply_tick, "sequence": 2}
	)
	...
	expect(
		not preclose.simulation.closed and preclose.simulation.tick == 2999,
		"the M4 checkpoint reaches one tick before closing"
	)
	...
	sequence.simulation.enqueue_command(
		{
			"kind": "set_priority",
			"target_id": "order_01",
			"value": 2,
			"apply_tick": sequence.simulation.tick + 2,
			"sequence": 1
		}
	)
	var sequence_restore: Dictionary = service_sim_script.call(
		"restore", sequence.definitions, sequence.simulation.export_state(), sequence.options
	)
	var next_sequence: Dictionary = sequence_restore.simulation.enqueue_command(
		{
			"kind": "set_priority",
			"target_id": "order_01",
			"value": 0,
			"apply_tick": sequence.simulation.tick + 3,
			"sequence": 2
		}
	)
	var repeated_sequence: Dictionary = sequence_restore.simulation.enqueue_command(
		{
			"kind": "set_priority",
			"target_id": "order_01",
			"value": 1,
			"apply_tick": sequence.simulation.tick + 4,
			"sequence": 1
		}
	)
	expect(
		next_sequence.accepted and not repeated_sequence.accepted,
...
func _prepared_service(service_sim_script: GDScript) -> Dictionary:
	...
	var commands: Array[Dictionary] = [
		{"kind": "set_prep", "target_id": "prepped_vegetable", "value": 1, "apply_tick": 0, "sequence": 1},
	...
	expect(
		(
			started.selection.prep_quantities.prepped_vegetable == 1
			and started.selection.placements.pass_01.tile == [8, 5]
		),
...
func _compare_restored_run(fixture: Dictionary, service_sim_script: GDScript, checkpoint: String) -> void:
	var simulation: RefCounted = fixture.simulation
	var restored: Dictionary = service_sim_script.call(
		"restore", fixture.definitions, simulation.call("export_state"), fixture.options
	)
	...
	expect(
		resumed.call("state_hash") == simulation.call("state_hash"),
	...
	while not simulation.get("closed"):
		...
		resumed.call("step")
	expect(
		resumed.call("state_hash") == simulation.call("state_hash"), "restore preserves the final hash: " + checkpoint
	)
...
func _test_shipped_snapshot_order(service_sim_script: GDScript) -> void:
	...
	for file_name: String in DirAccess.get_files_at(directory):
		...
		var started := PreparationPlan.new(scenario).apply_command(
		...
		expect(
			(
				ids.size() == started.definitions.order_schedule().size()
				and ids.size() < 100
				and ids == string_sorted
				and ids == _live_order_ids(simulation)
			),
...
func _test_three_digit_order_ids(service_sim_script: GDScript) -> void:
	...
	var started := PreparationPlan.new(data).apply_command(
		{"kind": "start", "target_id": "", "value": null, "apply_tick": 0, "sequence": 1}
	)
	...
	expect(
		(
			ids.size() == order_count
			and ids.find("order_99") < ids.find("order_100")
			and ids.find("order_100") < ids.find("order_105")
			and ids == live_ids
		),
		"the snapshot lists order_99 before order_100 and matches live order"
	)
	var restored: Dictionary = service_sim_script.call(
		"restore", started.definitions, simulation.call("export_state"), started.options
	)
	...
	expect(
		same_events and resumed.call("state_hash") == simulation.call("state_hash"),
...
func _fresh() -> Resource:
	return ResourceLoader.load(
		"res://content/campaign/scenarios/first_shift.tres", "", ResourceLoader.CACHE_MODE_IGNORE
	)
