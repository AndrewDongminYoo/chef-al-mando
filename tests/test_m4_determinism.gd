...
func _between_process_fixture(service_sim_script: GDScript, prepared: bool) -> Dictionary:
	...
	if prepared:
		...
		expect(plan.apply_command({"kind": "set_prep", "target_id": "prepped_vegetable", "value": 1,
...
func _prepared_service(service_sim_script: GDScript) -> Dictionary:
	...
	expect(started.selection.prep_quantities.prepped_vegetable == 1 and started.selection.placements.pass_01.tile == [8, 5],
