...
func run() -> void:
	...
	checks.expect(screen.settings_panel.settings_sound.button_pressed and screen.preferences.snapshot().sound_enabled
		and sound_toggles.is_empty(),
		"settings start with sound on and no toggle events")
	var requested_locale := 1 if locale == "en" else 0
	await _dialog_choose(screen.settings_panel.settings_locale, 1 - requested_locale, screen.safe_area.get_global_rect())
	...
	await _dialog_choose(screen.settings_panel.settings_text_size, 1 - requested_text_size, screen.safe_area.get_global_rect())
	checks.expect(screen.settings_panel.settings_sound.button_pressed and screen.preferences.snapshot().sound_enabled
		and sound_toggles.is_empty(),
		"opposite text-size input does not toggle sound")
	await _dialog_choose(screen.settings_panel.settings_text_size, requested_text_size, screen.safe_area.get_global_rect())
	checks.expect(screen.settings_panel.settings_sound.button_pressed and screen.preferences.snapshot().sound_enabled
		and sound_toggles.is_empty(),
	...
	checks.expect(not screen.settings_panel.settings_sound.button_pressed
	...
	await _dialog_click(screen.settings_panel.settings_sound, screen.safe_area.get_global_rect())
	settings_document = JSON.parse_string(FileAccess.get_file_as_string(directory + "/settings.json"))
	checks.expect(screen.settings_panel.settings_sound.button_pressed
