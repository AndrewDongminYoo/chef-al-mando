...
func run() -> void:
	...
	checks.expect(
		screen.safe_area.get_global_rect().encloses(screen.settings_button.get_global_rect()),
	...
	checks.expect(
		(
			screen.preferences.snapshot().locale == locale
	...
	checks.expect(
		checkpoint.accepted and checkpoint.active_session.simulation.tick == 100,
	...
	await process_frame
	checks.expect(
		screen.active_service == null and screen.continue_button.visible,
	...
	checks.expect(
		service != null and service.state == 2 and service.simulation.tick == 100,
	...
	checks.expect(
		screen.save_error_dialog.visible and screen.save_error_dialog.dialog_text.contains(expected_error),
	...
	checks.expect(
		screen.result_dialog.visible and screen.result_dialog.dialog_text.contains(expected_result),
...
func _capture_dense_service(directory: String) -> void:
	...
	for index: int in 5:
		...
		if played.accepted:
			checks.expect(
				progress.record_result(scenario.id, played.snapshot).passed,
				"dense render fixture earns prerequisite service: " + scenario.id
			)
	var save_path := directory + "/dense-campaign.json"
	checks.expect(
		CampaignStore.new(campaign, save_path).save_records(progress.snapshot().records).accepted,
...
func _wait_for_render_focus() -> void:
	...
	while Time.get_ticks_msec() < deadline_msec:
		...
		if root.has_focus() and Time.get_ticks_msec() - last_application_focus_msec >= 500:
			break
	checks.expect(
		root.has_focus() and Time.get_ticks_msec() - last_application_focus_msec >= 500,
...
func _safe_click(button: Button, allowed: Rect2) -> void:
	...
	for frame_index: int in range(4):
		...
		if (
			...
			and button.custom_minimum_size.y >= 64.0
			and current_allowed.encloses(button.get_global_rect())
		):
			break
		await process_frame
	if (
		not button.is_visible_in_tree()
		or button.disabled
		or button.custom_minimum_size.y < 64.0
		or not current_allowed.encloses(button.get_global_rect())
	):
		printerr(
			(
				"SAFE_TARGET visible=%s disabled=%s min_height=%s button=%s allowed=%s"
				% [
					button.is_visible_in_tree(),
					button.disabled,
...
func _dialog_click(button: BaseButton, allowed: Rect2, settle: bool = true) -> void:
	...
	for frame_index: int in range(4):
		if (
			button.is_visible_in_tree()
			and not button.disabled
			and button_rect.size.y >= 64.0
			and allowed.encloses(button_rect)
...
func _dialog_choose(picker: OptionButton, index: int, allowed: Rect2) -> void:
	...
	if not popup.visible or not opening_once:
		printerr(
			(
				"POPUP_OPEN selected_before=%d expected=%d pressed=%s about=%s hidden=%s popup=%s allowed=%s"
	...
	checks.expect(allowed.encloses(popup_rect), "settings selection popup fits inside the safe area")
	...
	checks.expect(item_center_spacing >= 64.0, "settings popup renders item centers at least 64 pixels apart")
	...
	if not popup.visible:
		...
		popup.popup_hide.disconnect(on_hidden)
		return
	checks.expect(not popup.get_item_text(index).is_empty(), "settings selection popup shows the requested choice")
	var local_point := (
		Vector2(popup.position) + Vector2(popup.size.x * 0.5, popup.size.y * (index + 0.5) / popup.item_count)
	)
	_move_pointer(local_point)
	checks.expect(popup.get_focused_item() == index, "settings popup pointer reaches item %d" % index)
	...
	for frame_index: int in range(4):
		if not popup.visible and dialog.has_focus():
			break
		await process_frame
	checks.expect(not popup.visible and dialog.has_focus(), "settings popup closes and returns input focus")
	checks.expect(hidden_frames.size() == 1, "settings selection closes one popup")
	picker.pressed.disconnect(on_pressed)
	popup.about_to_popup.disconnect(on_popup)
	popup.popup_hide.disconnect(on_hidden)
...
func _check_text(label: Label, description: String) -> void:
	checks.expect(
		label != null and label.is_visible_in_tree() and label.get_visible_line_count() >= label.get_line_count(),
