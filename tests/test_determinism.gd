...
func run(_tree: SceneTree) -> void:
	...
	expect(
		first.state_hash() == fast.state_hash(), "one and four speed give the same state with different frame intervals"
	)
	...
	expect(
		path_a.size() > 1 and path_a == b.path_between(Vector2i(2, 4), Vector2i(9, 4)),
		"the same obstacle grid produces the same tile path"
	)
...
func _test_pause_and_accumulator() -> void:
	...
	expect(
		sim.tick == 12 and driver.get("accumulator_us") == 50000, "a long frame preserves and executes every full tick"
	)
	driver.call("set_paused", true)
	var result := sim.enqueue_command(
		{"kind": "set_priority", "target_id": "order_01", "value": 2, "apply_tick": 13, "sequence": 1}
	)
	var second := sim.enqueue_command(
		{"kind": "set_priority", "target_id": "order_01", "value": 0, "apply_tick": 13, "sequence": 2}
	)
	...
	expect(
		(
			...
			and second.accepted
			and sim.tick == 12
			and sim.snapshot().orders[0].priority == 1
			and sim.snapshot().commands.size() == 2
		),
		"pause queues several commands without advancing their gameplay effects"
	)
	driver.call("set_paused", false)
	driver.call("advance_microseconds", 50000)
	expect(
		sim.tick == 13 and sim.snapshot().orders[0].priority == 0,
		"resume applies queued commands in sequence order and excludes paused wall time"
	)
