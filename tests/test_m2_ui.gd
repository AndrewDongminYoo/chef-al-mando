...
class CountingPlan:
	extends "res://sim/preparation_plan.gd"
	var snapshots: int = 0
...
func run(tree: SceneTree) -> void:
	...
	expect(
		HarnessSettingsStore.new(settings_path).save_settings(settings).accepted,
	...
	expect(employee_status.get_theme_font_size("font_size") == 18, "employee status uses the readable normal text size")
	expect(employee_status.max_lines_visible == 4, "employee status preserves translated destinations in dense layouts")
	expect(
		screen.get("app_preferences").update_settings({"text_size": "large"}).accepted,
		"employee status fixture enables large text"
	)
	expect(
		employee_status.get_theme_font_size("font_size") == 22, "employee status follows the readable large text size"
	)
	expect(
		screen.get("app_preferences").update_settings({"text_size": "normal"}).accepted,
	...
	expect(
		screen.get("summary_label").is_visible_in_tree() and screen.get("summary_label").text != initial_summary,
		"a purchase updates the visible preparation budget"
	)
	...
	expect(
		screen.get("definitions").stations[3].tile == Vector2i(8, 5),
		"an accepted placement updates the board definition"
	)
	...
	expect(
		not panel.visible and sim.call("snapshot").inventory.prepped_grill == 2,
		"start commits the prepared inventory and switches to service"
	)
	expect(
		screen.get("summary_label").is_visible_in_tree(), "service keeps its stock summary visible after preparation"
	)
	...
	expect(
		not screen.call("submit_preparation", "move_station", "pass_01", "left").accepted,
		"service refuses placement commands"
	)
	screen.call("advance", 1.0)
	expect(
		(
			screen.get("duty_labels")[0].text.contains("채소 샐러드")
			and screen.get("duty_labels")[0].text.contains("재료 보관대로 이동 중")
		),
		"the employee status names the menu and pickup destination"
	)
	screen.call("advance", 1.0)
	expect(
		screen.get("duty_labels")[0].text.contains("원재료 챙기는 중"),
		"the employee status distinguishes active raw ingredient pickup"
	)
	screen.call("advance", 1.0)
	expect(
		screen.get("duty_labels")[0].text.contains("손질하러 이동 중"),
	...
	expect(
		analysis_text.contains("다음 영업 추천") and analysis_text.contains("주문별 누적 시간") and analysis_text.contains("예약·사용"),
	...
	expect(
		action_index >= 0 and accounting_index > action_index and cumulative_index > action_index,
	...
	expect(
		int(analysis.get_meta("action_heading_font_size", 0)) > int(analysis.get_meta("action_font_size", 0)),
		"closing analysis makes the action heading larger than each proposed change"
	)
	expect(
		int(analysis.get_meta("action_font_size", 0)) == 18,
		"closing analysis keeps each proposed change at the readable body size"
	)
	expect(
		analysis.get_meta("action_color", Color.TRANSPARENT) == Color("eab06c"),
	...
	expect(
		(
			panel.visible
			and plan.call("snapshot").inventory.prepped_grill == 2
			and plan.call("snapshot").purchased_cost == 6600
		),
		"retry reconstructs the previous choices with fresh purchases"
	)
	...
	expect(
		(
			plan.call("snapshot").inventory.prepped_grill == 0
			and screen.get("definitions").stations[3].tile == Vector2i(9, 5)
		),
		"reset restores default preparation and layout"
	)
	screen.get("start_button").pressed.emit()
	screen.call("advance", 300.0)
	await tree.process_frame
	expect(
		screen.get("simulation").closed and analysis_scroll.scroll_vertical == 0,
...
func _expect_analysis_drag_scrolls(tree: SceneTree, scroll: ScrollContainer, content: Control) -> void:
	...
	await tree.process_frame
	expect(
		content.get_global_rect().has_point(scroll.get_global_rect().get_center()),
...
func _expect_korean_word_stays_whole(tree: SceneTree, screen: Control, content: RichTextLabel) -> void:
	...
	await tree.process_frame
	expect(
		content.get_parsed_text() == "버\u2060섯" and content.get_character_line(0) == content.get_character_line(2),
...
func _extra_menu(tree: SceneTree) -> void:
	...
	expect(
		(
			panel.get("prep_plus").has("prepped_grain_salad")
			and panel.get("prep_labels")["prepped_grain_salad"].text.contains("곡물 샐러드")
		),
		"a data-only menu receives a named preparation control"
	)
	panel.get("prep_plus")["prepped_grain_salad"].pressed.emit()
	screen.get("start_button").pressed.emit()
	screen.call("advance", 20.0)
	expect(
		screen.get("order_buttons")["order_01"].text.contains("곡물 샐러드"), "the fourth-menu order shows its resource name"
	)
	expect(
		screen.get("simulation").snapshot().orders[0].state == "served",
		"the displayed fourth-menu order actually serves"
	)
	screen.queue_free()
	await tree.process_frame
...
func _custom_ingredients(tree: SceneTree) -> void:
	for source_path: String in ["res://content/m1_first_service.tres", "res://content/m2_first_service.tres"]:
		...
		expect(
			ResourceSaver.save(data, fixture_path) == OK, "the custom ingredient fixture is saved for the real scene"
		)
		var screen := boot_main(tree, fixture_path)
		await tree.process_frame
		if data.call("supports_preparation"):
			expect(
				screen.get("preparation_panel").purchase_labels.greens.text.contains("잎채소"),
				"preparation names a renamed raw ingredient"
			)
		else:
			expect(
				(
					screen.get("summary_label").text.contains("잎채소 22")
					and screen.get("summary_label").text.contains("식용유 0")
				),
				"ready summary lists custom raw ingredients including those with no purchases"
			)
		screen.get("start_button").pressed.emit()
		screen.call("advance", 20.0)
		...
		expect(
			summary.contains("잎채소 %d" % snapshot.inventory.greens) and summary.contains("식용유 0"),
			"service summary shows the actual custom raw stock"
		)
		expect(
			not summary.split("\n프렙 · ")[0].contains("손질한"),
			"raw stock summary excludes prepared ingredient definitions"
		)
		screen.queue_free()
		await tree.process_frame
...
func _snapshot_per_tap(tree: SceneTree) -> void:
	...
	expect(
		plan.sequence() == 1 and view.sequence == 1 and view.purchases[ingredient_id] > purchased_before,
	...
	expect(
		screen.get("start_button").disabled == (not view.can_start),
