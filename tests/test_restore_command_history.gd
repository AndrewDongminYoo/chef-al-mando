...
func run(_tree: SceneTree) -> void:
	...
	for target_tick: int in [0, 1, 9]:
		...
		expect(
			state.schedule_cursor == 0 and state.last_sequence == 0 and state.tasks.is_empty(),
	...
	expect(
		queued.simulation.export_state().commands.size() == 1 and queued.simulation.export_state().last_sequence == 1,
	...
	expect(
		(
			early_duty.simulation.export_state().employees[0].duty == "all"
	...
	gap.simulation.step()
	expect(
		gap.simulation.export_state().last_sequence == 7 and gap.simulation.export_state().commands.size() == 1,
	...
	expect(
		(
			reordered.simulation.export_state().commands.size() == 1
	...
	for queued_only: bool in [false, true]:
		...
		expect(
			(
				state.orders[0].state == "working"
				and state.orders[0].priority == 1
	...
	for kind: String in ["set_priority", "set_duty", "cancel_order"]:
		...
		_reject(
			applied,
			corrupted,
			"invalid_employee" if kind == "set_duty" else "invalid_order",
			directory,
			"erased_sequence_" + kind
		)
	...
	expect(
		closed_state.last_sequence == 0 and closed_state.orders[0].state == "served",
...
func _fixture() -> Dictionary:
	...
	expect(
		(
			plan
			. apply_command(
				{"kind": "set_duty", "target_id": "employee_01", "value": "cold", "apply_tick": 0, "sequence": 1}
			)
			. accepted
		),
		"the fixture selects a nondefault first duty"
	)
	expect(
		(
			plan
			. apply_command(
				{"kind": "set_duty", "target_id": "employee_02", "value": "hot", "apply_tick": 0, "sequence": 2}
			)
			. accepted
		),
		"the fixture selects a nondefault second duty"
	)
	var started := plan.apply_command({"kind": "start", "target_id": "", "value": null, "apply_tick": 0, "sequence": 3})
	...
	return {
		...
		"simulation": ServiceSim.new(started.definitions, null, started.options)
	}
...
func _queue(fixture: Dictionary, kind: String, target: String, value: Variant, sequence: int, apply_tick: int) -> void:
	expect(
		(
			...
			. simulation
			. enqueue_command(
				{"kind": kind, "target_id": target, "value": value, "sequence": sequence, "apply_tick": apply_tick}
			)
			. accepted
		),
		"the fixture submits a real command: " + kind
	)
...
func _advance_to_work(fixture: Dictionary) -> void:
	while fixture.simulation.tick < 300:
		...
		if not state.orders.is_empty() and state.orders[0].state == "working":
			expect(
				state.employees[0].order_id == "order_01" and state.employees[0].pending_duty.is_empty(),
...
func _reject(fixture: Dictionary, corrupted: Dictionary, reason: String, directory: String, label: String) -> void:
	...
	expect(
		not rejected.accepted and rejected.reason == reason,
		"direct restore rejects command history corruption: " + label
	)
	...
	expect(
		store.save_active_session(session, {}).accepted and store.save_active_session(session, {}).accepted,
	...
	expect(
		restored.accepted and restored.simulation.state_hash() == before,
...
func _round_trip(fixture: Dictionary, label: String) -> void:
	...
	expect(
		restored.accepted and restored.simulation.state_hash() == fixture.simulation.state_hash(),
