...
func fresh() -> Definitions:
	return (
		ResourceLoader.load("res://content/m1_first_service.tres", "", ResourceLoader.CACHE_MODE_IGNORE) as Definitions
	)
...
func _test_invalid_load() -> void:
	...
	expect(
		view.closed and not view.errors.is_empty(), "a missing ingredient definition returns a readable rejected state"
	)
	expect(
		view.accounting is Dictionary and view.accounting.is_empty(),
		"invalid content must not calculate or display accounting"
	)
	sim.call("step")
...
func _test_process_boundaries() -> void:
	...
	expect(
		snapshot(sim).inventory.vegetable == 22 and snapshot(sim).reserved.vegetable == 1,
		"assignment reserves without consuming"
	)
	advance(sim, 10)
	expect(order(sim).state == "moving" and not order(sim).raw_consumed, "arrival does not start work on the same tick")
	advance(sim, 11)
	expect(
		order(sim).state == "working" and order(sim).completion_tick == 21,
		"pickup starts after arrival and lasts ten ticks"
	)
	expect(
		snapshot(sim).inventory.vegetable == 21 and snapshot(sim).reserved.vegetable == 0,
		"pickup consumes one reserved ingredient"
	)
	advance(sim, 21)
	expect(
		order(sim).phase_id == "cook" and order(sim).state == "moving",
		"pickup releases its station before cooking movement"
	)
	advance(sim, 36)
	expect(order(sim).state == "working" and order(sim).completion_tick == 96, "cooking starts after three grid tiles")
	advance(sim, 126)
	expect(
		order(sim).phase_id == "serve" and order(sim).completion_tick == 136,
		"serving includes six tiles and ten work ticks"
	)
	advance(sim, 136)
	expect(
		order(sim).state == "served" and snapshot(sim).accounting.revenue == 500,
		"completed service awards revenue once"
	)
	expect(snapshot(sim).inventory.vegetable == 21, "later processes do not consume raw ingredients again")
	advance(sim, 3000)
	expect(
		snapshot(sim).accounting.profit == -8100 and snapshot(sim).accounting.cash == 1900,
		"profit subtracts purchases and labor once"
	)
...
func _test_cancel_and_last_ingredient() -> void:
	...
	advance(sim, 2)
	expect(
		order(sim).state == "cancelled" and snapshot(sim).tasks.is_empty(), "cancellation releases moving assignment"
	)
	expect(
		snapshot(sim).inventory.vegetable == 22 and snapshot(sim).reserved.vegetable == 0,
		"cancel before pickup restores availability"
	)
	expect(
		snapshot(sim).employees[0].progress == 0 and snapshot(sim).employees[0].tile == [2, 4],
		"moving cancellation restores the edge origin"
	)
	...
	advance(sim, 12)
	expect(
		snapshot(sim).inventory.vegetable == 21 and snapshot(sim).accounting.waste_cost == 100,
		"cancel after pickup retains consumed loss"
	)
	expect(
		(
			snapshot(sim).tasks.is_empty()
			and snapshot(sim).reserved.values().all(func(value: int) -> bool: return value == 0)
		),
		"cancel after consumption releases every reservation"
	)
	var data := single()
	...
	expect(
		snapshot(sim).tasks.is_empty() and snapshot(sim).orders.size() == 2 and snapshot(sim).reserved.vegetable == 0,
		"two waiting orders reach the same assignment pass with the last ingredient unreserved"
	)
	command(sim, "set_duty", "employee_01", "all", 4, 3)
	command(sim, "set_duty", "employee_02", "all", 4, 4)
	advance(sim, 4)
	expect(
		snapshot(sim).tasks.size() == 1 and snapshot(sim).reserved.vegetable == 1,
		"last ingredient belongs to only one assignment"
	)
	expect(order(sim, 1).wait_reason == "missing_ingredients", "second employee cannot claim a reserved ingredient")
	advance(sim, 139)
	expect(
		snapshot(sim).inventory.vegetable == 0 and order(sim, 1).state == "waiting",
		"last ingredient is consumed only once"
	)
	data = single()
	data.menu_ids = ["soup"]
	data.order_count = 2
	...
	expect(
		(
			...
			and snapshot(sim).reserved.vegetable == 2
			and snapshot(sim).reserved.grain == 1
		),
		"missing grain prevents a partial vegetable reservation for the second soup"
	)
	advance(sim, 11)
	expect(
		snapshot(sim).inventory.vegetable == 2 and snapshot(sim).inventory.grain == 0,
		"the soup shortage fixture actually exhausts grain while vegetables remain"
	)
	expect(
		(
			snapshot(sim).reserved.values().all(func(value: int) -> bool: return value == 0)
			and snapshot(sim).tasks.size() == 1
		),
		"grain shortage leaves no partial reservation or assignment"
	)
...
func _test_expiration_and_closing() -> void:
	...
	advance(sim, 135)
	expect(
		order(sim).state == "working" and order(sim).completion_tick == 136,
		"deadline fixture reaches serving before the collision"
	)
	advance(sim, 136)
	expect(
		order(sim).state == "expired" and order(sim).terminal_reason == "deadline",
		"deadline wins over same-tick service completion"
	)
	expect(
		snapshot(sim).accounting.revenue == 0 and snapshot(sim).tasks.is_empty(),
		"expiration releases tasks without revenue"
	)
	data = single()
	...
	advance(sim, 136)
	expect(
		order(sim).terminal_reason == "service_closed" and snapshot(sim).accounting.revenue == 0,
		"closing wins over same-tick completion"
	)
	expect(
		snapshot(sim).accounting.waste_cost == 6600 and snapshot(sim).accounting.profit == -8600,
		"closing loss is not subtracted twice"
	)
...
func _test_assignment_and_commands() -> void:
	...
	expect(
		order(sim).wait_detail == "responsible_employee_busy",
		"a responsible but occupied employee has a distinct detail"
	)
	sim = _waiting_pair()
	command(sim, "set_duty", "employee_01", "all", 4, 3)
	advance(sim, 4)
	...
	advance(sim, 4)
	expect(
		order(sim).deadline_tick == order(sim, 1).deadline_tick and snapshot(sim).tasks[0].order_id == "order_01",
		"equal deadlines use the stable order ID"
	)
	sim = _waiting_pair()
	...
	for invalid: Array in [
		...
		expect(
			not result.accepted and sim.call("state_hash") == before,
			"invalid command must not mutate state: " + str(invalid)
		)
	command(sim, "cancel_order", "order_01", null, 5, 5)
	advance(sim, 5)
	before = sim.call("state_hash")
	expect(
		not command(sim, "set_priority", "order_01", 2, 6, 6).accepted and sim.call("state_hash") == before,
		"terminal orders reject commands without mutation"
	)
...
func _test_handoff_and_path_failure() -> void:
	...
	advance(sim, 2)
	expect(
		snapshot(sim).employees[0].duty == "all" and snapshot(sim).employees[0].pending_duty == "off",
		"duty remains pending during movement"
	)
	advance(sim, 21)
	expect(
		snapshot(sim).employees[0].duty == "off" and snapshot(sim).tasks[0].employee_id == "employee_02",
		"duty changes after the current process"
	)
	expect(
		order(sim).result_position == [2, 2] and not order(sim).carrying, "handoff preserves the pickup result position"
	)
	...
	expect(
		...
		"runtime path failure releases the assignment"
	)
	expect(
		order(sim).result_position == [2, 2] and snapshot(sim).inventory.vegetable == 21,
		"failure before collection preserves the result and consumed stock"
	)
	data = single()
	...
	advance(sim, 37)
	expect(
		order(sim).result_position == origin and not order(sim).carrying,
		"failure after collection drops the result at the recovered tile"
	)
	expect(
		snapshot(sim).inventory.vegetable == 21 and snapshot(sim).reserved.vegetable == 0,
		"path recovery never restores consumed ingredients"
	)
	data = single()
	routes = GridRoutes.new(data.grid_size, data.blocked_tiles())
	sim = sim_script.new(data, routes)
	...
	advance(sim, 2)
	expect(
		snapshot(sim).reserved.vegetable == 0 and snapshot(sim).inventory.vegetable == 22,
		"failure before consumption releases raw reservations"
	)
	advance(sim, 136)
...
func _test_default_service() -> void:
	...
	for next_tick: int in range(1, 3001):
		...
		for task: Dictionary in view.tasks:
			valid = (
				valid
				and not workers.has(task.employee_id)
				and not stations.has(task.station_id)
				and not positions.has(task.work_position)
			)
		...
		for ingredient_id: String in view.inventory:
			valid = (
				valid
				and view.reserved[ingredient_id] >= 0
				and view.reserved[ingredient_id] <= view.inventory[ingredient_id]
			)
	...
	expect(
		view.tasks.is_empty() and view.reserved.values().all(func(value: int) -> bool: return value == 0),
		"closing releases every task and ingredient reservation"
	)
	print(
		"M1_BASELINE ",
		JSON.stringify({"served": served, "accounting": view.accounting, "hash": sim.call("state_hash")})
	)
...
func _test_work_positions_and_cooking_duty() -> void:
	...
	advance(sim, 4)
	expect(
		snapshot(sim).tasks.size() == 1 and order(sim, 1).wait_reason == "station_in_use",
		"different station IDs cannot reserve the same work position twice"
	)
	sim = sim_script.new(single())
	advance(sim, 36)
	...
	expect(
		(
			snapshot(sim).employees[0].duty == "all"
			and snapshot(sim).employees[0].pending_duty == "off"
			and order(sim).completion_tick == 96
		),
		"the last duty request waits without changing active cooking"
	)
	advance(sim, 96)
	expect(
		snapshot(sim).employees[0].duty == "off" and snapshot(sim).tasks[0].employee_id == "employee_02",
		"the next employee receives the serving process after cooking finishes"
	)
