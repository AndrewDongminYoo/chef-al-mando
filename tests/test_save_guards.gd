...
func run(_tree: SceneTree) -> void:
	...
	_records = {"first_shift": {"completed": true, "best_served": first.minimum_served,
	...
	expect(ServiceSession.restore(_campaign, _session, _records).accepted
...
func _test_single_guards() -> void:
	...
	_expect_reason(_save_session(target, "not a session", _records), "invalid_session",
		"save_active_session rejects a session that is not a dictionary")
	_expect_reason(_save_session(target, _session, {}), "invalid_session",
		"save_active_session rejects a session that the records lock")
	_expect_reason(_save_records(target, {}), "invalid_session",
		"save_records rejects records that lock the stored session")
	...
	for version: Array in [["future", "schema_version", 99, "future_version"],
		["unsupported", "sim_version", 0, "unsupported_version"]]:
...
func _test_guard_order() -> void:
	...
	_write(target, "{broken primary")
	_expect_both(target, "invalid_records", {"unknown_service": {}}, _session, null,
		"records are checked before the files")
	_expect_reason(_save_session(target, _session, {}), "invalid_session",
		"save_active_session checks the new session before it reads the files")
	_expect_reason(_save_records(target, {}), "corrupt_records",
		"save_records reads the files before it checks the stored session")
	_expect_both(target, "corrupt_records", _records, _session, BAD_ATTEMPTS,
		"attempts are checked after the files")

	target = _valid_files("order_future_backup")
	_write(target, "{broken primary")
	_write(target + ".backup", JSON.stringify(_document("schema_version", 99)))
	_expect_both(target, "future_version", _records, _session, null,
		"a protected backup is reported before a corrupt primary")

	target = _valid_files("order_locked_session")
	_expect_reason(_save_records(target, {}, BAD_ATTEMPTS), "invalid_session",
		"save_records checks the stored session before the attempts")
	_expect_reason(_save_session(target, _session, {}, BAD_ATTEMPTS), "invalid_session",
...
func _expect_both(target: String, reason: String, records: Dictionary, session: Variant, attempts: Variant,
	label: String) -> void:
...
func _save_session(target: String, session: Variant, records: Dictionary, attempts: Variant = null) -> Dictionary:
	var before := _bytes(target)
	return _unchanged(target, before, CampaignStore.new(_campaign, target).save_active_session(session, records, attempts))


func _unchanged(target: String, before: Array, result: Dictionary) -> Dictionary:
	if not result.accepted:
		expect(_bytes(target) == before,
...
func _valid_files(name: String) -> String:
	...
	expect(store.save_active_session(null, _records).accepted
		and store.save_active_session(_session, _records).accepted,
