...
func run(tree: SceneTree) -> void:
	...
	expect(
		(
			screen.get("continue_button").visible
	...
	expect(_find_button(screen, "설정") != null, "the campaign catalog provides a settings action")
	...
	expect(
		(
			campaign_locale_popup.get_theme_font_size("font_size") == 26
	...
	expect(
		(
			screen.get("save_label").text
			== "Kitchen rules were updated. Completion records are preserved, and the service in progress can continue."
		),
	...
	expect(
		(
			screen.get("menu_title").get_theme_font_size("font_size") > base_font
	...
	expect(
		(
			campaign_locale_popup.get_theme_font_size("font_size") == 32
	...
	expect(
		service_settings != null and service.get_node("SafeArea/Layout/Controls/Start").text == "Start",
	...
	expect(
		(
			service_locale_popup.get_theme_font_size("font_size") == 32
	...
	expect(
		(
			service.get_node("SafeArea/Layout/Header/Title").text == "First service"
	...
	expect(
		service.get("simulation").state_hash() == unchanged_state and service.get("simulation").tick == 0,
	...
	expect(
		not rejected_preparation.accepted and service.get("feedback_label").text.contains("Budget"),
		"an English preparation rejection shows its translated reason"
	)
	service.get("settings_locale").item_selected.emit(0)
	expect(
		(
			service.get("feedback_label").text.contains("예산이 부족합니다")
	...
	expect(
		(
			service.get("feedback_label").text.contains("Budget")
			and service.get("simulation").state_hash() == unchanged_state
		),
	...
	expect(
		loaded.accepted and loaded.active_session is Dictionary and loaded.active_session.simulation.tick == 0,
	...
	if service_settings != null:
		...
		expect(
			not service.call("is_running") and service.get("settings_dialog").visible and not arrival_player.playing,
			"opening settings pauses the simulation and stops an actual arrival cue"
		)
		service.get("settings_dialog").hide()
		expect(not service.call("is_running"), "closing service settings does not resume the simulation")
	...
	expect(
		(
			loaded.accepted
			and loaded.active_session.simulation.tick == 110
	...
	expect(
		service.call("submit_command", "set_duty", "employee_01", "all").accepted,
		"the pause checkpoint fixture queues a real command"
	)
	expect(
		service.get("feedback_label").text.contains("commands queued"),
		"an accepted English command shows a translated queued state"
	)
	service.call("advance", 0.15)
	expect(
		service.get("feedback_label").text == "Command applied",
		"the English queued state becomes applied after its command leaves the queue"
	)
	service.get("pause_button").pressed.emit()
	loaded = CampaignStore.new(screen.get("campaign"), save_path).load_records()
	expect(
		loaded.active_session.simulation.tick == 112 and loaded.active_session.simulation.last_sequence == 1,
	...
	service.get("pause_button").pressed.emit()
	expect(
		service.call("submit_command", "set_duty", "employee_01", "cold").accepted,
	...
	expect(
		screen.get("active_service") == null, "confirming the menu action leaves the paused service in its checkpoint"
	)
	screen.queue_free()
	await tree.process_frame
	await tree.process_frame
	...
	expect(
		continue_button != null and continue_button.visible,
	...
	if continue_button != null:
		...
		expect(
			service.call("submit_command", "set_priority", "order_01", 2).accepted,
		...
		expect(
			(
				loaded.active_session.simulation.closed
		...
		expect(
			(
				screen.get("result_dialog").dialog_text.contains("Served")
		...
		expect(
			FileAccess.get_file_as_bytes(save_path) == closed_bytes,
		...
		loaded = CampaignStore.new(screen.get("campaign"), save_path).load_records()
		expect(
			not loaded.active_session.simulation.closed,
	...
	if continue_button != null:
		...
		expect(
			(
				screen.get("active_service").get("state") == 3
...
func _test_failed_checkpoint_pauses_live_service(tree: SceneTree, entry: String, directory: String) -> void:
	...
	expect(
		(
			screen.get("pending_save")
			and screen.get("save_error_dialog").visible
			and service.get("state") == 2
			and service.get("driver").paused
	...
	expect(
		service.get("simulation").tick == failed_tick,
	...
	expect(service.get("simulation").tick == failed_tick, "a successful Start retry waits for manual resume")
	service.get("resume_button").pressed.emit()
	await tree.create_timer(0.15).timeout
	expect(
		service.call("is_running") and service.get("simulation").tick > failed_tick,
	...
	await tree.create_timer(0.1).timeout
	expect(service.get("simulation").tick == failed_tick, "a successful automatic retry waits for manual resume")
	service.get("resume_button").pressed.emit()
	await tree.create_timer(0.15).timeout
	expect(
		service.call("is_running") and service.get("simulation").tick > failed_tick,
...
func _test_failed_checkpoint_and_replacement(tree: SceneTree, entry: String, directory: String) -> void:
	...
	expect(
		screen.get("pending_save") and retry != null and screen.get("save_label").text.contains("다시 시도"),
		"an open-service save failure shows its reason and a visible retry action"
	)
	screen.call("request_menu")
	expect(
		(
			screen.get("active_service") == service
			and not screen.get("leave_dialog").visible
			and screen.get("save_error_dialog").visible
		),
	...
	expect(
		(
			screen.get("save_label").text.contains("another save attempt")
	...
	expect(screen.get("save_label").text.contains("다시 시도"), "the retained storage failure translates back to Korean")
	screen.call("return_to_menu")
	expect(screen.get("active_service") == service, "a failed open-service save blocks direct navigation")
	failed_store.failure = ""
	if retry != null:
		retry.pressed.emit()
	expect(
		not screen.get("pending_save") and FileAccess.file_exists(file_path),
	...
	expect(
		(
			screen.get("pending_save")
			and screen.get("save_error_dialog").visible
			and not screen.get("goal_dialog").visible
			and not screen.get("leave_dialog").visible
		),
	...
	screen.get("service_goal_button").pressed.emit()
	expect(
		not screen.get("pending_save") and screen.get("goal_dialog").visible and not service.call("is_running"),
	...
	expect(
		(
			screen.get("pending_save")
			and screen.get("save_error_dialog").visible
			and not service.get("settings_dialog").visible
			and not screen.get("goal_dialog").visible
		),
	...
	service.get("settings_button").pressed.emit()
	expect(
		not screen.get("pending_save") and service.get("settings_dialog").visible and not service.call("is_running"),
	...
	screen.get("leave_dialog").confirmed.emit()
	await tree.process_frame
	expect(screen.get("active_service") == null, "the menu action succeeds after the checkpoint retry")
	...
	expect(
		screen.get("active_service") == null and replace != null,
		"starting over with an open checkpoint requires an explicit replacement confirmation"
	)
	expect(
		screen.get("replace_dialog").dialog_text == "새 영업에서 시작을 누르면 저장한 이어하기를 새 영업으로 교체합니다.",
		"replacement copy names Start as the checkpoint replacement point"
	)
	screen.get("settings_locale").item_selected.emit(1)
	expect(
		screen.get("replace_dialog").dialog_text == "Starting the new service replaces the saved checkpoint.",
	...
	expect(
		(
			...
			and screen.get("continue_button").visible
			and screen.get("active_session") == saved_session
			and _stored_session(file_path) == stored_session
		),
	...
	expect(
		service.call("submit_preparation", "set_purchase", "vegetable", replacement_purchase).accepted,
...
func _test_lifecycle_audio(tree: SceneTree, entry: String, directory: String) -> void:
	...
	service.get("lifecycle").call("_notification", Node.NOTIFICATION_APPLICATION_FOCUS_OUT)
	expect(
		not service.call("is_running") and not arrival.playing,
	...
	expect(
		not service.call("is_running") and service.get("simulation").tick == paused_tick,
		"focus return does not resume service time"
	)
	service.get("resume_button").pressed.emit()
	service.call("advance", 21.0)
	expect(arrival.playing, "manual resume enables only a later order-arrival cue")
	var later_arrival_tick: int = service.get("simulation").tick
	service.get("lifecycle").call("_notification", Node.NOTIFICATION_APPLICATION_FOCUS_OUT)
	service.get("lifecycle").call("_notification", Node.NOTIFICATION_APPLICATION_FOCUS_IN)
	expect(
		not arrival.playing and not service.call("is_running") and service.get("simulation").tick == later_arrival_tick,
	...
	expect(
		(
			service.get("state") == 3
			and service.get("simulation").closed
	...
	expect(
		(
			not served.playing
			and service.get("state") == 3
			and service.get("simulation").tick == closed_tick
			and closed_checkpoints.value == 0
		),
	...
	service.get("lifecycle").call("_notification", Node.NOTIFICATION_APPLICATION_FOCUS_IN)
	expect(
		service.get("state") == 0 and service.get("simulation").tick == 0 and ready_checkpoints.value == 0,
	...
	expect(
		service.call("is_running") and service.get("simulation").tick == 10 and ready_arrival.playing,
...
func _test_quit_request_checkpoint(tree: SceneTree, entry: String, directory: String) -> void:
	var requests := {
		"window close": Node.NOTIFICATION_WM_CLOSE_REQUEST, "back gesture": Node.NOTIFICATION_WM_GO_BACK_REQUEST
	}
	for label: String in requests:
		...
		expect(
			(
				not service.call("is_running")
				and service.get("simulation").tick == live_tick
				and after.accepted
				and after.active_session.simulation.tick == live_tick
			),
...
func _test_service_locale_refresh(tree: SceneTree, entry: String, directory: String) -> void:
	...
	preparation_panel.get("prep_plus")["prepped_vegetable"].pressed.emit()
	...
	expect(
		(
			preparation_panel.visible
			and preparation_panel.get("pages")[1].visible
			and employee_one_label != null
			and employee_one_label.is_visible_in_tree()
			and employee_two_label != null
			and employee_two_label.is_visible_in_tree()
			and ready_preparation.prep_quantities.prepped_vegetable == 1
	...
	expect(
		service.get("state") == 0 and service.get("shown_tenths") == 0 and service.get("simulation").tick == 0,
	...
	expect(
		remaining.text == "Time remaining 300.0 s  ·  ",
		"a ready locale change refreshes the exact English remaining time"
	)
	expect(
		tr("프렙 재료 받으러 재료 보관대로 이동 중") == "fetching prep" and tr("원재료 받으러 재료 보관대로 이동 중") == "fetching raw",
		"English pickup movement stays concise in dense employee status columns"
	)
	expect(employee_one_label.text == "Employee 1", "a ready locale change refreshes the exact first employee name")
	expect(employee_two_label.text == "Employee 2", "a ready locale change refreshes the exact second employee name")
	expect(
		preparation_panel.get("pages")[1].visible and service.get("preparation").snapshot() == ready_preparation,
	...
	expect(remaining.text == "남은 시간 300.0초  ·  ", "a ready locale change refreshes the exact Korean remaining time")
	expect(employee_one_label.text == "직원 1", "the Korean refresh restores the exact first employee name")
	expect(employee_two_label.text == "직원 2", "the Korean refresh restores the exact second employee name")
	expect(
		preparation_panel.get("pages")[1].visible and service.get("preparation").snapshot() == ready_preparation,
	...
	expect(
		service.call("is_running") and service.get("compact_layout") and details_toggle.visible,
	...
	expect(
		(
			service.get("state") == 2
			and service.get("shown_tenths") == 10
			and service.get("simulation").tick == 10
			and details_toggle.visible
		),
		"the paused locale fixture has a warm displayed time cache and a visible compact control"
	)
	expect(
		not detail_panel.visible and details_toggle.text == "주문 상세 펼치기",
	...
	expect(
		remaining.text == "Time remaining 299.0 s  ·  ",
		"a paused locale change refreshes the exact English remaining time"
	)
	expect(
		details_toggle.text == "Show order details", "a paused locale change refreshes the collapsed details control"
	)
	expect(
		(
			service.get("state") == 2
			...
			and service.get("simulation").state_hash() == paused_hash
	...
	expect(remaining.text == "남은 시간 299.0초  ·  ", "a paused locale change refreshes the exact Korean remaining time")
	expect(details_toggle.text == "주문 상세 펼치기", "the Korean refresh restores the collapsed details control text")

	details_toggle.pressed.emit()
	expect(
		detail_panel.visible and details_toggle.text == "주문 상세 접기",
		"the paused compact fixture starts its expanded check with Korean text"
	)
	service.get("settings_locale").item_selected.emit(1)
	expect(details_toggle.text == "Hide order details", "a paused locale change refreshes the expanded details control")
	expect(
		(
			service.get("state") == 2
			and service.get("simulation").tick == 10
	...
	service.get("settings_locale").item_selected.emit(0)
	expect(details_toggle.text == "주문 상세 접기", "the Korean refresh restores the expanded details control text")
	expect(
		(
			service.get("state") == 2
			and service.get("simulation").tick == 10
	...
	expect(
		service.get("state") == 3 and service.get("shown_tenths") == 3000 and service.get("simulation").tick == 3000,
	...
	expect(
		remaining.text == "Time remaining 000.0 s  ·  ",
	...
	expect(remaining.text == "남은 시간 000.0초  ·  ", "a closed locale change refreshes the exact Korean remaining time")
	expect(
		(
			service.get("state") == 3
			and service.get("simulation").tick == 3000
	...
	expect(
		saved_preferences.update_settings({"locale": "en"}).accepted,
	...
	expect(
		TranslationServer.get_locale() == "en" and service.call("is_running") and service.get("simulation").tick == 0,
...
func _test_recovered_active_session(tree: SceneTree, entry: String, directory: String) -> void:
	...
	expect(
		store.save_active_session(session, {}).accepted and store.save_records({}).accepted,
	...
	expect(
		(
			recover_button.visible
			and session_only_button.visible
	...
	expect(
		(
			screen.get("storage_blocked")
			and screen.get("save_message_kind") == "storage"
	...
	screen.get("settings_locale").item_selected.emit(1)
	expect(
		recover_button.text == "Recover from backup",
		"a locale change refreshes the exact English backup recovery action"
	)
	expect(
		session_only_button.text == "Start without saving",
		"a locale change refreshes the exact English session-only action"
	)
	expect(
		screen.get("save_label").text == "The save could not be read. Recover the backup or start without saving.",
	...
	screen.get("settings_locale").item_selected.emit(0)
	expect(recover_button.text == "백업에서 복구", "the Korean refresh restores the exact backup recovery action")
	expect(session_only_button.text == "저장 없이 새로 시작", "the Korean refresh restores the exact session-only action")
	expect(
		screen.get("save_label").text == "저장 기록을 정상적으로 읽을 수 없습니다. 백업을 복구하거나 저장 없이 시작할 수 있습니다.",
	...
	expect(continue_button.visible, "recovering a backup exposes its open service as a continue action")
	continue_button.pressed.emit()
	await tree.process_frame
	var service: Control = screen.get("active_service")
	expect(
...
func _test_invalid_campaign_settings(tree: SceneTree, entry: String, directory: String) -> void:
	...
	for index: int in fixtures.size():
		...
		expect(
			ResourceSaver.save(fixtures[index], campaign_path) == OK,
		...
		expect(
			(
				screen.get("progress") == null
				and screen.get("store") == null
		...
		expect(
			(screen.get("campaign") == null) == (index == 0),
			"fixtures cover both a failed campaign cast and failed campaign validation"
		)
		screen.get("settings_button").pressed.emit()
		expect(screen.get("settings_dialog").visible, "settings remain accessible after campaign initialization fails")
		screen.get("settings_locale").item_selected.emit(1)
		expect(
			screen.get("save_label").text == "Could not load campaign data",
		...
		expect(
			(
				fresh_preferences.load_settings().accepted
				and fresh_preferences.snapshot() == {"locale": "en", "sound_enabled": false, "text_size": "large"}
			),
		...
		expect(
			(
				screen.get("save_label").text == "캠페인 데이터를 불러올 수 없습니다"
				and screen.get("scenario_buttons").is_empty()
				and screen.get("begin_button").disabled
				and not screen.call("begin_service")
				and not FileAccess.file_exists(records_path)
			),
...
func _test_rejected_close_and_single_preference_apply(tree: SceneTree, entry: String, directory: String) -> void:
	...
	expect(
		(
			screen.get("last_result").is_empty()
	...
	expect(
		not screen.get("result_dialog").visible and screen.get("service_goal_button").text == "목표",
	...
	expect(
		per_application == 1 and probe.get("calls") == 2 * per_application,
...
func _test_rejected_resume_result(tree: SceneTree, entry: String, directory: String) -> void:
	...
	expect(
		(
			...
			and service != null
			and service.get("state") == 3
			and screen.get("last_result").is_empty()
			and screen.get("service_goal_button").text == "목표"
...
func _test_future_settings_error(tree: SceneTree, entry: String, directory: String) -> void:
	...
	file.store_string(
		JSON.stringify({"schema_version": 99, "locale": "en", "sound_enabled": false, "text_size": "large"})
	)
	file.close()
	...
	expect(
		(
			screen.get("settings_message").text.contains("지원하지 않는 설정")
	...
	screen.get("settings_locale").item_selected.emit(1)
	expect(
		FileAccess.get_file_as_bytes(settings_path) == original, "settings input cannot overwrite future settings bytes"
	)
	screen.queue_free()
	await tree.process_frame
	await tree.process_frame
...
func _test_operational_option_popups(tree: SceneTree, entry: String, directory: String) -> void:
	var screen := _boot(
		tree, entry, directory + "/operational-popups.json", directory + "/operational-popups-settings.json"
	)
	await tree.process_frame
	screen.get("begin_button").pressed.emit()
	var service: Control = screen.get("active_service")
	...
	expect(
		_operational_popups_match(preparation_pickers, 26) and _operational_popups_match(live_pickers, 26),
		"normal text styles preparation and live duty popup rows at 26 pixels"
	)
	service.get("settings_text_size").item_selected.emit(1)
	expect(
		_operational_popups_match(preparation_pickers, 32) and _operational_popups_match(live_pickers, 32),
	...
	service.get("start_button").pressed.emit()
	expect(
		service.call("is_running") and _operational_popups_match(live_pickers, 26),
	...
	expect(
		service.get("state") == 0 and _operational_popups_match(preparation_pickers, 26),
...
func _test_session_only_checkpoint_watermark(tree: SceneTree, entry: String, directory: String) -> void:
	...
	service.checkpoint_requested.connect(
	...
	expect(
		(
			screen.get("session_only")
			and service.get("last_saved_tick") == 0
	...
	expect(
		(
			automatic_ticks == [100, 200]
			and service.get("last_saved_tick") == 200
			and FileAccess.get_file_as_bytes(file_path) == original_bytes
		),
	...
	for _tick: int in 99:
		service.call("_process", 0.1)
	expect(
		automatic_ticks == [100, 200] and service.get("last_saved_tick") == 250,
		"a session-only pause preserves the automatic checkpoint cadence"
	)
	service.call("_process", 0.1)
	expect(
		(
			automatic_ticks == [100, 200, 350]
			and service.get("last_saved_tick") == 350
			and FileAccess.get_file_as_bytes(file_path) == original_bytes
		),
	...
	expect(
		automatic_count_before_restart == 29 and automatic_ticks[-1] == 2950,
	...
	expect(
		(
			automatic_ticks.size() == automatic_count_before_restart + 1
			and automatic_ticks[-1] == 100
...
func _popup_row_height(popup: PopupMenu) -> int:
	...
	var radio_height := maxi(
		popup.get_theme_icon("radio_checked").get_height(), popup.get_theme_icon("radio_unchecked").get_height()
	)
...
func _operational_popups_match(pickers: Array[OptionButton], font_size: int) -> bool:
	...
	for picker: OptionButton in pickers:
		...
		if (
			popup.get_theme_font_size("font_size") != font_size
