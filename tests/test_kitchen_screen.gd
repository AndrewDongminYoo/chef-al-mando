...
func run() -> void:
	...
	checks.expect(
		screen.preparation_panel.pages[0].get_global_rect().encloses(screen.summary_label.get_global_rect()),
		"the preparation budget fits inside the visible stock scroll area"
	)
	await RenderingServer.frame_post_draw
	checks.expect(
		root.get_texture().get_image().save_png("res://build/check/kitchen-screen-preparation.png") == OK,
		"the preparation budget saves a rendered frame"
	)
	for command: Dictionary in Policies.reference_policy("split_duties").preparation:
		checks.expect(
			screen.submit_preparation(command.kind, command.target_id, command.value).accepted,
	...
	checks.expect(
		not screen.detail_panel.visible and screen.details_toggle.visible,
		"phone service starts with order details collapsed"
	)
	checks.expect(
		_cell_size(screen.board, screen.definitions.grid_size) >= 36.0,
		"the rendered 14 by 9 kitchen keeps cells at least 36 pixels"
	)
	checks.expect(
		screen.duty_buttons.size() == 4 and screen.summary_label.get_line_count() >= 3,
	...
	checks.expect(
		normal_board_rect == board_rect,
		(
			"changing to large text does not resize the phone kitchen board: %s -> %s, kitchen %s -> %s"
			% [normal_board_rect, board_rect, normal_kitchen_rect, large_kitchen_rect]
		)
	)
	...
	if not working.is_empty():
		checks.expect(
			screen.submit_command("set_duty", working.id, "off").accepted,
		...
		checks.expect(
			refreshed.pending_duty == "off" and employee_text.contains("현재 공정 후 담당 변경"),
			"the rendered staff label shows the pending responsibility change"
		)
		checks.expect(
			employee_text.contains(activity),
	...
	checks.expect(
		board_rect == refreshed_board_rect,
		(
			"wrapped stock, selected details, and pending duty text do not resize the kitchen board: %s -> %s"
			% [board_rect, refreshed_board_rect]
		)
	)
	...
	await _settle(screen)
	checks.expect(
		screen.detail_panel.visible and board_rect == screen.board.get_global_rect(),
		"opening the selected order detail does not resize the kitchen board"
	)
	await RenderingServer.frame_post_draw
	var image := root.get_texture().get_image()
	checks.expect(
		not image.is_empty() and image.save_png("res://build/check/kitchen-screen-dense.png") == OK,
	...
	(
		checks
		. expect(
			screen.state == screen.State.CLOSED and screen.board.size.y > service_board_height,
			(
				"analysis expands the board after the service staff controls are hidden: state=%s tick=%s height=%s -> %s paused=%s"
				% [
					screen.state,
					screen.simulation.tick,
					service_board_height,
					screen.board.size.y,
					screen.driver.paused
				]
			)
		)
	)
	checks.expect(
		screen.analysis_label.accessibility_name.contains("손익"), "analysis keeps its accounting summary visible"
	)
	screen.queue_free()
	await process_frame
...
func _settle(screen: Control) -> void:
	...
	screen._apply_safe_area(
		Rect2i(inset, 0, int(screen.size.x) - inset * 2, int(screen.size.y) - 30), Transform2D.IDENTITY
	)
	for frame_index: int in 4:
		await process_frame
...
func _assert_directional_work_pixels() -> void:
	...
	for direction: String in ["north", "east", "south", "west"]:
		...
		station.tile = {
			"north": Vector2i(6, 3), "east": Vector2i(7, 4), "south": Vector2i(6, 5), "west": Vector2i(5, 4)
		}[direction]
		...
		var snapshot := {
			...
			[
				{
					"id": "employee_01",
					"tile": [work_tile.x, work_tile.y],
					"next_tile": [work_tile.x, work_tile.y],
					"progress": 0,
					"duty": "all",
					"order_id": "order_01"
				}
			],
			"orders": [{"id": "order_01", "recipe_id": "salad", "phase_id": "cook", "state": "working"}],
			"tasks":
			[{"employee_id": "employee_01", "station_id": station.id, "work_position": [work_tile.x, work_tile.y]}]
		}
		...
		var crop := root.get_texture().get_image().get_region(
			Rect2i(Vector2i(center) - Vector2i.ONE * crop_size / 2, Vector2i.ONE * crop_size)
		)
	...
	checks.expect(
		hashes.size() == 4 and unique.size() == 4,
...
func _assert_english_pending_layout(tablet: bool) -> void:
	...
	if not working.is_empty():
		checks.expect(
			screen.submit_command("set_duty", working.id, "off").accepted,
			"English fixture requests a duty change during work"
		)
		screen.advance(0.1)
		await _settle(screen)
		checks.expect(
			_employee_by_id(screen.latest_view.employees, working.id).pending_duty == "off",
			"English layout fixture actually has a pending duty"
		)
		checks.expect(
			screen.board.get_global_rect() == before,
			"English pending duty cannot resize or move the board: %s -> %s" % [before, screen.board.get_global_rect()]
		)
		var viewport_bounds := root.get_visible_rect()
		checks.expect(
			viewport_bounds.encloses(screen.board.get_global_rect()),
			"English pending duty keeps the board inside the actual viewport"
		)
		checks.expect(
			viewport_bounds.encloses(screen.get_node("SafeArea/Layout/Kitchen/Body/Side").get_global_rect()),
			"English pending duty keeps the side panel inside the actual viewport"
		)
		checks.expect(
			viewport_bounds.encloses(screen.get_node("SafeArea/Layout/Header").get_global_rect()),
			(
				"English pending duty keeps the header inside the actual viewport: %s header=%s screen=%s safe=%s"
				% [
					viewport_bounds,
					screen.get_node("SafeArea/Layout/Header").get_global_rect(),
					screen.size,
					screen.safe_area.get_global_rect()
				]
			)
		)
		checks.expect(
			viewport_bounds.encloses(screen.pause_button.get_global_rect()),
			"English pending duty keeps time controls inside the actual viewport"
		)
		checks.expect(
			viewport_bounds.encloses(screen.duty_buttons[-1].get_global_rect()),
			"English pending duty keeps staff controls inside the actual viewport"
		)
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png(
			"res://build/check/kitchen-screen-%s-en-pending.png" % ("tablet" if tablet else "phone")
		)
		if not tablet:
			screen.details_toggle.pressed.emit()
		await _settle(screen)
		checks.expect(
			screen.detail_panel.visible and screen.board.get_global_rect() == before,
			"English large-text details keep the same board area"
		)
		var stable := true
		...
		checks.expect(
			screen.simulation.tick == activity_start + 600 and activity_texts.size() > 1,
			"the activity fixture advances one minute and changes rendered staff text"
		)
		checks.expect(
			stable, "English board remains stable through a minute of actual activity changes: tablet=%s" % tablet
		)
	screen.queue_free()
	await process_frame
