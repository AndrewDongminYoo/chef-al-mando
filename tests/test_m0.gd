...
func run(tree: SceneTree) -> void:
	...
	expect(screen.get("input_actions") == 1, "one start signal is recorded before state guards")
	screen.advance(1.0)
	expect(screen.elapsed_seconds == 1.0, "start advances the counter")
	screen.pause_button.pressed.emit()
	expect(screen.get("input_actions") == 2, "one pause signal adds one input action")
	screen.advance(10.0)
	expect(screen.elapsed_seconds == 1.0, "pause freezes the counter")
	screen.resume_button.pressed.emit()
	expect(screen.get("input_actions") == 3, "one resume signal adds one input action")
	screen.advance(1.0)
