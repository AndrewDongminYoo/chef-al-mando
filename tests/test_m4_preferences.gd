...
func _test_store(file_path: String) -> void:
	...
	expect(
		fresh.accepted and fresh.reason == "loaded" and fresh.values == values,
	...
	for invalid: Dictionary in [
		...
		expect(
			(
				...
				and rejected.reason == "invalid_settings"
	...
	for failure: String in ["write", "invalid_temp", "replace"]:
		...
		expect(
			SettingsStore.new(target).save_settings(values).accepted,
		...
		expect(
			not failed.accepted and FileAccess.get_file_as_bytes(target) == bytes,
			"a settings write failure preserves primary bytes: " + failure
		)
	_write(
		file_path, JSON.stringify({"schema_version": 99, "locale": "en", "sound_enabled": false, "text_size": "large"})
	)
	...
	expect(
		(
			...
			and protected.reason == "future_version"
			and FileAccess.get_file_as_bytes(file_path) == original_bytes
		),
	...
	_write(
		file_path,
		JSON.stringify(
			{"schema_version": 2, "locale": "en", "sound_enabled": false, "text_size": "large", "added_later": true}
		)
	)
	original_bytes = FileAccess.get_file_as_bytes(file_path)
	...
	expect(
		replaced.accepted and reloaded.accepted and reloaded.values == values,
...
func _test_failed_load_locale(file_path: String) -> void:
	for fixture: Dictionary in [
		...
		{
			"label": "future",
			"text": '{"schema_version":99,"locale":"en","sound_enabled":false,"text_size":"large"}',
		...
		expect(
			not loaded.accepted and loaded.reason == fixture.reason,
			"a failed preference load returns its original failure: " + fixture.label
		)
		expect(
			preferences.snapshot() == {"locale": "ko", "sound_enabled": true, "text_size": "normal"},
			"a failed preference load publishes the literal default snapshot: " + fixture.label
		)
		expect(
			TranslationServer.get_locale() == "ko",
...
func _test_preferences(tree: SceneTree, file_path: String) -> void:
	...
	var loaded: Dictionary = preferences.load_settings()
	expect(
		loaded.accepted and preferences.snapshot().locale == "ko" and TranslationServer.get_locale() == "ko",
	...
	expect(
		restored_load.accepted and restored.snapshot().locale == "en" and TranslationServer.get_locale() == "en",
	...
	expect(
		(
			root.get_theme_font_size("font_size") == 24
			and static_label.get_theme_font_size("font_size") == 18
			and root.scale == Vector2.ONE
			and static_label.scale == Vector2.ONE
		),
		"large text changes static font sizes without scaling controls"
	)
	preferences.apply_to(root)
	expect(
		root.get_theme_font_size("font_size") == 24 and static_label.get_theme_font_size("font_size") == 18,
	...
	expect(
		(
			dynamic_label.get_theme_font_size("font_size") == preferences.font_size(dynamic_base)
	...
	expect(
		(
			dialog_body != null
			and dialog_body.get_theme_font_size("font_size") == preferences.font_size(dialog_body_base)
			and confirmation_body != null
			and confirmation_body.get_theme_font_size("font_size") == preferences.font_size(confirmation_body_base)
			and (
				confirmation.get_cancel_button().get_theme_font_size("font_size")
				== preferences.font_size(cancel_button_base)
			)
		),
	...
	expect(
		(
			normalized.accepted
			and root.get_theme_font_size("font_size") == 20
			and static_label.get_theme_font_size("font_size") == 15
			and dynamic_label.get_theme_font_size("font_size") == dynamic_base
			and not dynamic_label.has_theme_font_size_override("font_size")
			and themed_label.get_theme_font_size("font_size") == 13
			and themed_button.get_theme_font_size("font_size") == 17
			and confirmation_body != null
			and confirmation_body.get_theme_font_size("font_size") == confirmation_body_base
			and confirmation.get_cancel_button().get_theme_font_size("font_size") == cancel_button_base
			and not themed_label.has_theme_font_size_override("font_size")
			and not themed_button.has_theme_font_size_override("font_size")
		),
	...
	expect(
		not rejected.accepted and change_count.value == 2 and preferences.snapshot().sound_enabled,
...
func _test_translation() -> void:
	...
	expect(
		(
			TranslationServer.translate("Chef al Mando · 영업 목록") == "Chef al Mando · Service list"
