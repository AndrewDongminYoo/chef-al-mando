...
func _capture(tablet: bool, locale: String, text_size: String) -> void:
	...
	checks.expect(
		(
			...
			. new(directory + "/settings.json")
			. save_settings({"locale": locale, "sound_enabled": false, "text_size": text_size})
			. accepted
		),
		"capture settings are saved"
	)
	...
	await _settle(screen, tablet)
	checks.expect(screen.settings_panel.settings_dialog.visible, "settings fixture opens the actual dialog")
	_check_dialog(screen.settings_panel.settings_dialog, screen.safe_area.get_global_rect())
	await _frame("settings")
	screen.settings_panel.settings_dialog.hide()
	...
	await _settle(service, tablet)
	_check_dialog(service.settings_panel.settings_dialog, service.safe_area.get_global_rect())
	await _frame("service-settings")
	service.settings_panel.settings_dialog.hide()
	...
	if service.details_toggle.visible:
		...
		checks.expect(
			service.details_toggle.text == TranslationServer.translate(expected_toggle),
	...
	await _settle(service, tablet)
	checks.expect(
		screen.result_dialog.visible and service.simulation.snapshot().closed, "result fixture closes a real service"
	)
	...
	checks.expect(
		(
			analysis_text.find(TranslationServer.translate("다음 영업 추천")) >= 0
			and (
				analysis_text.find(service.summary_label.text)
				> analysis_text.find(TranslationServer.translate("다음 영업 추천"))
			)
		),
...
func _capture_completed_campaign(screen: CampaignScreen, tablet: bool) -> void:
	for scenario: Resource in screen.campaign.scenarios:
		...
		checks.expect(
			played.accepted and screen.progress.record_result(scenario.id, played.snapshot).passed,
			"completed catalog fixture earns its record: " + scenario.id
		)
	checks.expect(
		screen.store.save_records(screen.progress.snapshot().records).accepted,
		"completed catalog fixture saves real campaign records"
	)
	screen.return_to_menu()
	await _settle(screen, tablet)
	checks.expect(
		screen.ending_button.visible and screen.continue_button.visible,
	...
	checks.expect(
		screen.select_scenario("split_duties") and screen.begin_service(),
		"dense fixture opens the four-employee service"
	)
	var service := screen.active_service
	service.set_process(false)
	for command: Dictionary in Policies.reference_policy("split_duties").preparation:
		checks.expect(
			service.submit_preparation(command.kind, command.target_id, command.value).accepted,
	...
	await _settle(service, tablet)
	checks.expect(
		service.duty_buttons.size() == 4 and service.board.size.y > 0, "dense fixture renders staff and the kitchen"
	)
...
func _settle(screen: Control, tablet: bool) -> void:
	...
	screen.call(
		"_apply_safe_area",
		Rect2i(inset.x, 0, int(screen.size.x) - inset.x * 2, int(screen.size.y) - inset.y),
...
func _capture_cancel_states(service: Control) -> void:
	...
	checks.expect(
		not hover_color.is_equal_approx(pressed_color), "rendered cancel press is visibly different from hover"
	)
	...
	checks.expect(
		service.get("simulation").state_hash() == before, "cancel state capture does not submit an order command"
	)
...
func _check_cancel_color(button: Button) -> Color:
	...
	var point := (
		(button.get_global_rect().position + Vector2(20, 10)) * Vector2(frame.get_size()) / root.get_visible_rect().size
	)
	var pixel := frame.get_pixelv(Vector2i(point))
	checks.expect(
		pixel.r > pixel.g + 0.06 and pixel.r > pixel.b + 0.06,
...
func _check_target(button: Button, allowed: Rect2) -> void:
	checks.expect(
		(
			...
			and button.size.x >= 64
			and button.size.y >= 64
			and allowed.encloses(button.get_global_rect())
		),
		"product target fits inside the safe area: " + button.text
	)


func _check_text(label: Label) -> void:
	checks.expect(
		label.is_visible_in_tree() and label.get_visible_line_count() >= label.get_line_count(),
...
func _check_dialog(dialog: AcceptDialog, allowed: Rect2) -> void:
	...
	checks.expect(
		(
			dialog.visible
			and allowed.encloses(Rect2(dialog.position, dialog.size))
			and allowed.encloses(close_rect)
			and close_rect.size.y >= 64
		),
...
func _frame(state: String) -> void:
	...
	var frame := root.get_texture().get_image()
	checks.expect(
		not frame.is_empty() and frame.save_png("res://build/check/polish/%s-%s.png" % [variant, state]) == OK,
