...
func run(_tree: SceneTree) -> void:
	...
	var pressure_services: Array[String] = [
		"hot_queue", "shared_stock", "long_route", "split_duties", "rush_hour", "final_service"
	]
	for scenario: Resource in scenarios:
		...
		expect(
			schedule.size() == scenario.order_count and schedule[-1].arrival_tick < 3000,
			"all configured orders arrive before closing: " + scenario.id
		)
		...
		expect(
			scenario.employees.size() <= 4 and scenario.stations.size() <= 6,
			"service stays within mobile resource caps"
		)
		if scenario.id in pressure_services:
			_test_pressure_service(scenario, schedule)
	expect(
		menus.size() == 8 and ingredients.size() <= 12,
		"campaign has eight menus and at most twelve ingredient definitions"
	)
	...
	expect(
		changed_scenario.call("validate").is_empty(), "the closing-tick schedule fixture starts from a valid service"
	)
	...
	expect(
		"campaign arrival schedule must be ordered within service time" in changed_scenario.call("validate"),
...
func _test_pressure_service(scenario: Resource, schedule: Array) -> void:
	...
	expect(
		arrival_ticks is PackedInt32Array and arrival_ticks.size() == scenario.order_count,
	...
	for index: int in arrival_ticks.size():
		expect(
			schedule[index].arrival_tick == arrival_ticks[index],
	...
	expect(
		fixed_roles.has("storage") and fixed_roles.has("pass"),
