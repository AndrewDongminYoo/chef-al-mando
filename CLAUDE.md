...
- `persistence/campaign_store.gd` owns the save document and its version stamp (`VERSIONS`: schema, content and sim versions) plus the readable older versions. `persistence/service_session.gd` serializes an in-progress `ServiceSim`, and every read re-validates it with `ServiceSession.restore`.
