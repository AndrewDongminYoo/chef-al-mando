...
- Static content is `Resource` data under `content/campaign/` (`campaign.tres` plus `scenarios/`, `recipes/` and `ingredients/`), with the definition scripts such as `content/campaign_def.gd` one level up; per-attempt order menus come from `content/schedule_generator.gd` using the scenario's `forecast_slack` and a seed.
