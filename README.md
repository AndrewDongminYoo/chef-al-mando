...
| 저장소         | 8개 영업과 엔딩, 저장·이어하기, 한·영·글자 크기·효과음, 오프라인 라이선스와 메뉴별 기본 우선순위를 구현했습니다. 후반 6개 영업은 주문 파동과 고정 설비 공간 규칙을 사용합니다. |
| 엔진           | Godot Standard(버전은 저장소 루트의 `.godot-version`이 소유), 같은 버전의 export template, Compatibility 렌더러를 사용합니다.                                                  |
| 마일스톤       | M5 로컬 준비와 메뉴별 우선순위 보완본의 iPhone 플레이를 확인했습니다. 전체 출시 수용 검증은 남아 있습니다.                                                                     |
| 화면·아이콘    | 공통 화면 테마와 제품 아이콘, Android 적응형 아이콘 보정을 완료했습니다. [검증 기록](docs/notes/product-polish-verification.md)의 iPhone 실기기 확인이 남아 있습니다.          |
| 다음 단계      | 새 주문 압력과 시나리오별 전략 차이를 실기기에서 다시 플레이해 확인합니다. Android 실기기·사용자 5명 검증은 보류입니다.                                                        |
...
2. 시나리오에 정해진 메뉴의 기본 우선순위, 재료, 프렙, 직원 담당, 설비 배치를 정합니다.
...
환경 변수에는 `GODOT_BIN=/usr/local/bin/godot`을 등록합니다.
...
`check.sh mise`는 예보 폭·시드 생성기·세션과 기록의 시드 보존을 검사하며, 시드 0은 작성된 일정과의 항등을, 여섯 압력 영업의 시도 1~5는 추첨 인지 기준 정책의 통과와 무계획의 미달을 확인합니다(`docs/specs/pressure-rebalance.md` §4).
