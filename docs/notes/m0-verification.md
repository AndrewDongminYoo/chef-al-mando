...
## 2026-09-14 통합 iPhone 관찰

소스 `1c7d4a232438ad65be9bc575e44fdff5f9779d12`로 만든 iOS 개발 빌드를 iPhone 16 Pro의 기존 앱 위에 설치하고 실행했습니다.
기기는 iOS 26.6.1 (`23G83`)이었습니다.
운영자는 통합 체크리스트의 양 가로 방향·노치와 가장자리·버튼·글자 표시, 실제 터치, 10초 배경 전환, 복귀 후 정지 유지와 명시적 재개를 모두 통과했다고 보고했습니다.
이번 실행은 파일 로그 옵션 없이 시작했으므로 손가락 탭 횟수와 `M0_INPUT` 횟수 대조는 아직 남아 있습니다.
설치·산출물·저장 보존의 상세 식별자는 [M5 검증 기록](m5-verification.md)의 아홉 번째 절에 있습니다.
