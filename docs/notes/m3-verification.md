...
## 2026-09-14 스테이지별 세 전략 후속 검증

운영자는 각 스테이지마다 목표를 통과하는 실질적으로 다른 방법을 최소 3개 확보하도록 요구했습니다.
기존 콘텐츠 값과 저장 버전은 바꾸지 않고, 기준 정책 1개와 대체 정책 2개가 서로 다른 플레이어 선택을 제출하도록 정책 fixture를 확장했습니다.
각 정책은 실제 영업을 마감까지 실행해 제공 수와 손익 목표를 모두 통과합니다.
같은 스테이지의 세 최종 상태 해시는 쌍별로 다르며, 각 정책의 반복 실행과 1배속·4배속 실행은 동일한 해시를 냅니다.
스테이지별 실제 선택과 제공·손익 결과는 [캠페인 운영 압력 검증](kitchen-pressure-verification.md)의 세 전략 표에 기록했습니다.

테스트를 먼저 확장한 RED 실행은 1,051개 검사 중 스테이지별 정책 수 부족 8건으로 실패했습니다.
대체 정책을 고정한 GREEN 실행은 1,101개 검사, 실패 0으로 통과했습니다.
이 결과는 세 경로의 재현 가능한 도달 가능성을 보호하며, 사용자가 세 경로를 스스로 발견하거나 충분히 다르게 느끼는지는 별도 사용자 관찰입니다.

Oracle의 `strategy diversity` 조회는 대상·메커니즘·원인 가정을 구분하라는 글로벌 휴리스틱을 반환했으며 출처는 `wiki/concepts/strategy-diversity-enforcement.md`입니다.
캠페인 전용 선례가 아니므로 플레이 규칙으로 복제하지 않고 선택과 최종 상태를 함께 검증하는 방향만 확인했습니다.
`campaign policies`의 프로젝트 전용 선례는 `[no precedent found]`였습니다.
