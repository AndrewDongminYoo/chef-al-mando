...
2026-09-09에 원본 PR #7을 드래프트로 전환하고 저장 핵심 PR #8과 종속 화면 통합 PR로 분할했습니다.
분할 원본은 `3325af04d3a1f196aaf3743d9bf75351a904b69a`입니다.
[분할 계획](../plans/m4-pr-split.md)에 따라 두 PR의 실제 머지가 확인된 뒤에만 원본 PR을 닫습니다.
저장 검증 보완과 새 회귀는 [저장 핵심 검증 기록](m4-storage-core-verification.md)에 있습니다.
아래 단계별·렌더링·리뷰 기록은 원본 작업의 이력이며 새 PR의 현재 커밋 검사나 새 렌더링으로 해석하지 않습니다.

분할 후 `build/check/split-ui-final-m0.log`부터 `split-ui-final-m4.log`는 M0 25·M1 177·M2 378·M3 616·M4 1061건, 실패 0건을 기록했습니다.
`split-ui-final-m4-core.log`의 저장 핵심 855건도 실패 없이 통과했습니다.
이 검사는 상태·실제 Control·이벤트·파일 읽기를 검사하는 헤드리스 실행이며 렌더링 증거는 아닙니다.
`split-ui-final-restart.log`는 별도 프로세스의 상태·설정 복원 6건을 통과했습니다.
`split-ui-final-export-harness.log`는 저장 핵심과 화면 통합의 완료 표시 누락을 각각 거부하는 9건을 통과했습니다.
`split-ui-final-pck.log`는 새 PCK에서 저장 핵심·영업 이어하기·영어 표시를 포함한 완료 표시 여섯 개를 확인했습니다.

원본의 변경 경로 58개 중 45개가 바이트 단위로 일치하며, 나머지 13개는 저장 관계 보완·검사 연결·문서 변경입니다.
비교기는 메모리 안에서 `presentation/main.gd`를 변형한 입력을 먼저 거부했습니다.
독립 리뷰 두 건은 화면의 명령·저장 연결과 QA·CI의 저장 핵심 유지 여부를 각각 확인하고 승인했습니다.
루트는 원본 대비 공통 파일 차이를 별도로 검토했습니다.
지정 경로 CSpell은 32개 파일·건너뜀 3개에서 문제 0건, Trunk는 47개 파일에서 문제 0건을 보고했습니다.
이 결과는 코드·문서의 정적 검사이며 화면의 실제 표시를 증명하지 않습니다.

분할 후 AGENTS 실행 기준 변경에 대해 `codex doctor --summary --ascii --no-color`를 실행했습니다.
환경 진단은 경고 1건·실패 0건이며 update 구성 경고가 남았습니다.
원본은 `build/check/split-ui-codex-doctor.log`이고 게임 동작의 검증 근거는 아닙니다.

PR #8의 첫 hosted 코드 리뷰는 저장 핵심 명령과 블루프린트의 `[UNKNOWN]` 표기 충돌 한 건을 보고했습니다.
`bb6c730`에서 블루프린트에 `m4-core`·export 명령과 검증 범위를 연결했고, 화면 PR에도 이 문서 수정을 합류했습니다.
PR #9의 첫 hosted 코드 리뷰는 일러스트 주방 직원 번호에 큰 글자 배율이 빠진 P2 한 건을 보고했습니다.
번호를 그리는 `draw_string` 호출에서 기본 크기를 계산한 뒤 `text_scale`을 곱하도록 한 줄을 수정했습니다.
따라서 앞의 바이트 일치 결과는 이 후속 수정 전 추출 시점의 기록입니다.
...
## PR 후속 리뷰의 복구 버튼과 직원 이름

`ef05f20` 리뷰에서 저장 오류 화면의 복구 버튼과 준비 탭의 직원 이름이 언어 변경 뒤 이전 문구를 유지하는 결함을 확인했습니다.
손상된 주 파일과 정상 세션 백업을 실제로 읽어 두 복구 버튼을 표시한 뒤 한국어→영어→한국어 문구를 확인합니다.
이 전환은 손상 상태·실패 이유·주 파일과 백업 바이트를 보존하고 자동 복구를 수행하지 않습니다.
직원 이름은 실제 프렙 수량과 담당을 선택한 배치 탭에서 읽으며, 언어 전환 뒤 전체 준비 snapshot과 열린 탭이 유지되는지도 확인합니다.

캠페인의 문구 갱신에 두 복구 버튼을 포함하고, 준비 화면의 직원 Label을 기존 방식대로 보관해 정의의 이름으로 다시 번역합니다.
수정 전 영어 복구 버튼 두 개와 직원 이름 두 개만 실패했으며, 별도 파일의 영어 설정으로 시작하는 검사는 기존에도 통과했습니다.
`build/check/m4-round5-locale-refresh-red.log`는 M4 619건·실패 4건, `m4-round5-locale-refresh-green.log`는 M4 619건·실패 0건입니다.
