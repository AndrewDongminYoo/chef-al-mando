...
메뉴별 기본 우선순위를 정하면 새 주문에 자동으로 적용됩니다.
...
Set a default priority for each menu item before service.
New orders use this priority.
...
| 아이콘               | `assets/branding/app-icon.png`를 제품 아이콘 원본으로 등록했습니다. 실제 후보 빌드의 플랫폼 아이콘 검증은 별도로 기록합니다. |
