...
## 2026-09-14 iPhone 화면·아이콘 관찰

소스 `1c7d4a232438ad65be9bc575e44fdff5f9779d12`로 만든 개발 앱을 iPhone 16 Pro에 설치한 뒤 운영자가 통합 체크리스트를 수행했습니다.
운영자는 홈 화면 앱 아이콘이 잘리거나 비정상적으로 축소되지 않았고, 양 가로 방향에서 화면·버튼·글자가 노치나 가장자리에 가려지지 않았다고 확인했습니다.
한국어·영어와 큰 글자를 적용한 캠페인 목록·준비 화면에서도 주요 버튼이 잘리지 않았으며 실제 터치가 정상이라고 보고했습니다.
이 결과는 iPhone 한 대의 지정 화면을 확인하며 Android 런처·실제 태블릿·스토어 화면 이미지를 검증하지 않습니다.
