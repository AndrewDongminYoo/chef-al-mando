...
2026-09-08 운영자는 최종 M1 iPhone 설치본의 정상 동작을 승인했습니다.
Android 실기기 검증은 보류하며 개별 탭 횟수나 마감 수치를 새로 계측한 것으로 해석하지 않습니다.
...
| iPhone M1-12         | 최종 설치본 정상 동작을 운영자가 승인, 개별 계측 수치는 미제공 |
...
이 시점에는 실제 탭 횟수와 M1 완주 결과의 운영자 확인을 기다렸습니다.
새 빌드에서도 `Mouse is not supported by this display server.` 엔진 오류가 기록됐으므로 해소됐다고 판단하지 않습니다.
이후 2026-09-08 00:12 KST에 최종 `a4517e1` 빌드를 기존 앱 위에 설치했고 `devicectl`은 성공을 반환했습니다.
설치 결과는 당시 작업 디렉터리의 `build/check/m1-iphone-install-a4517e1.json`에 있습니다.
운영자는 설치 후 정상 동작을 확인하고 승인했으며 [PR #3에 승인 기록](https://github.com/AndrewDongminYoo/chef-al-mando/pull/3#issuecomment-5572738174)을 남겼습니다.
검토 head `45090bf`의 CI·코드·보안 리뷰가 완료됐고 미해결 스레드는 없었습니다.
PR은 `5388ebc`로 머지돼 있으며 검토 head와 파일 트리가 같습니다.
