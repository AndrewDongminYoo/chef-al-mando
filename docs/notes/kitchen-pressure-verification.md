...
2026-09-22부터 `../specs/pressure-rebalance.md` §4가 이 두 문장을 대체합니다(여분 2건·3,000, 무계획 미달 폭은 시도 1–5에도 적용).
...
이 표의 여섯 압력 영업 값은 아래 영업별 "2026-09-21 재조율" 절이 대체하며, 권위는 그 절과 `.tres`·`m3_policies.gd`에 있습니다.
...
2026-09-22 정정: 이 문장은 낡았습니다 — 두 영업 모두 수렴했고, `final_service`는 위 절의 2차 소절(상한 22)이, 이 영업은 아래 3차 소절(원래 구성)이 권위입니다.
...
#### 2026-09-22 2차: 구성 재설계 (수렴 실패, 측정만)

운영자가 2026-09-22에 이 영업의 구성 재설계(명세 §5의 3단계: `order_recipe_ids`·`order_arrival_ticks`·`order_count`, 메뉴 8종 유지, 목표는 올리기만)를 승인해 Task 9b로 다시 쟀습니다.
결과는 수렴하지 않아 `content/campaign/scenarios/rush_hour.tres`·`tests/fixtures/m3_policies.gd`·`translations/en.po`는 바꾸지 않았고, 이 소절만 남깁니다.
측정은 scratchpad 탐침 `rh_candidates.gd`로 했습니다: 캠페인의 `rush_hour`를 `duplicate()`해 후보 JSON의 구성·발주(주지 않으면 시드 0 필요량)·slack(주지 않으면 `max(1, n / 5)`, 곧 여덟 메뉴 각 1)·상한·목표를 넣고, 시도 0–5마다 `Policies.run_policy`로 무계획·기준(프렙 + 우선순위, 발주는 `draw_aware_policy`와 같은 값)·프렙만·우선순위만·발주만을 돌리며, 프렙 집합 × 우선순위 사전 가족을 여섯 시도에 대고(`RH_MAPS`), `--sweep`이면 프렙 조합을 전수하고(`--noprio`로 우선순위 제거, `--allrows`로 미달 행까지 기록), `--stations`면 시드 0의 설비 작업 구간을 tick 단위로 찍습니다.
현재 구성을 이 탐침에 넣은 보정 실행은 위 1바퀴 게이트 표 여섯 행(24/9,200 · 19/1,750 · 21/5,800 · 24/9,700 · 20/4,300 · 19/3,450)과 시드 0의 프렙만 21/4,000·우선순위만 21/6,100·무계획 21/4,650을 그대로 재현했습니다.
후보 선별은 `rh_score.py`(여섯 시도 최솟값을 목표로 두었을 때 §4.1 여분·§4.2 무계획 폭·1층 지렛대·시드 0의 프렙만 대리값을 한꺼번에 판정)로 했고, 통과한 후보만 커밋된 스윕 도구의 `--items` 상한(`marinated_protein:3,prepped_vegetable:6,prepped_grain:6,prepped_mushroom:6,soup_base:3,thawed_protein:3`)으로 프렙만 전수 스윕을 돌려 확인했습니다.
후보 생성기·선별기·로그는 모두 세션 scratchpad에 있고 커밋하지 않았습니다.

##### 게이트가 요구하는 산술

구성 하나가 수렴하려면 기준 정책(프렙 + 우선순위 사전)과 목표 (S, P)가 다음을 동시에 만족해야 합니다.
시도 0–5의 기준 정책 제공·손익이 모두 (S, P) 이상이고, S ≥ 23·P ≥ 8,500이며(목표는 올리기만), 시드 0의 기준 정책이 S + 2·P + 3,000 이하이고, 무계획이 모든 시도에서 제공 2건 이상 또는 손익 1,500원 이상 미달이며, 시드 0에서 프렙만 스윕의 모든 조합이 (S, P) 가운데 하나에 미달해야 합니다.
앞의 넷은 기준 정책의 여섯 시도 편차가 제공 2건·손익 3,000원 안이어야 한다는 뜻이고, 마지막은 프렙만의 최댓값이 그 편차의 아래쪽 끝보다 낮아야 한다는 뜻입니다.
이번 측정이 보인 것은 이 둘이 이 주방에서 서로를 밀어낸다는 사실입니다: 편차가 작은 구성은 프렙만으로도 닿고, 프렙만으로 닿지 않는 구성은 여섯 시도 최솟값이 23건 아래로 떨어집니다.

##### 주방의 실제 비용(tick 탐침)

설계 산술은 화구 둘 × 인내 500 tick = 1,000 tick이 파동 하나의 온식 조리 용량이라는 데서 출발했지만, 설비 작업 구간 탐침이 보인 실제 비용은 걷기와 직원 배정입니다.
같은 tick에 온식 넷이 오는 파동(`UG`@10, `SSGP`@160, `L`@310; 기준 프렙 `marinated_protein 3, prepped_vegetable 4, prepped_grain 2, prepped_mushroom 2, thawed_protein 1` + 구이·덮밥 2)의 시드 0 구간은 다음과 같습니다.
집기는 재료 보관대 하나에서 한 건씩 10 tick이라 직렬이고(10 tick 도착 둘: 15–25·25–35, 160 tick 도착 넷: 직원 3 혼자 215–255), 보관대 → 냉식대 25 tick·보관대 → 화구 45 tick·화구 → 보관대 복귀 50 tick 안팎을 걸으며, 공정마다 주문을 놓고 다른 직원이 걸어와 집으므로 재운 연어로 손질을 건너뛴 구이도 집기 235 뒤 화구에 오르는 것은 390(화구 2가 280에 비었지만, 집은 직원 3은 다음 집기로 넘어갔고 화구 2에서 양송이 수프를 마친 직원 2가 보관대까지 걸어와 집어 돌아온 뒤)이며 조리 630·제공 650으로 마감 660을 겨우 지키고, 같은 배치의 수프 둘은 손질(280–340·300–330)을 마치고도 화구 차례가 오지 않아 660에 만료됩니다.
직원은 목록 순서로 배정되어(`_try_assignment`가 빈 직원과 빈 설비의 첫 조합을 택함) 직원 1·2가 대부분을 맡고 직원 4는 셋이 모두 묶인 뒤에야 움직여, 위 파동을 프렙만으로 돌리면 직원 4의 첫 작업이 285 tick(160 tick 배치의 수프 집기)이고, 3+3 배치 후보(`LAS`@10, `UGP`@160)를 `marinated_protein 3, prepped_grain 3, prepped_vegetable 3, thawed_protein 3`으로 돌리면 580 tick(520 tick 도착 샐러드의 집기)입니다.
그래서 현재 구성에서도 주문 하나의 체류는 300–500 tick이고(시드 0 바닥 프렙에서 10 tick 수프가 340에, 520 tick 덮밥이 970에 나감), 같은 tick에 온식 4–6건을 모으면 어떤 정책으로도 그 가운데 2–3건만 인내 안에 나갑니다(아래 H 가족의 `MRUUGP`@520은 기준 정책이 M·G·P를 내고 R·U·U를 잃었고, `SGGGP`@1540은 G 하나만 냈습니다).
곧 명세가 말하는 "두 화구가 인내 안에 못 끝내는 온식 부하"는 온식 넷부터 이미 생기지만, 그 부하는 우선순위로 비싼 주문을 고르는 폭을 주는 대신 파동당 제공을 2–3건 깎습니다.

##### 후보 가족과 결과

후보 407가지를 열여덟 로그로 쟀고(아래 표는 모양이 같은 로그를 한 줄로 묶음), 묶음마다 (프렙, 우선순위 사전) 조합 가운데 여섯 시도 최솟값이 가장 높은 것과 그 조합의 시드 0 값, 같은 프렙의 시드 0 프렙만 값(대리값: 우선순위를 뺀 같은 프렙), 판정을 적습니다.
"통과"는 위 산술 전부를 대리값으로 만족한 조합 수이고, slack은 적지 않은 한 여덟 메뉴 각 1입니다.

| 묶음                        | 후보 | 모양                                               | 최고 조합의 여섯 시도 최솟값 | 시드 0      | 프렙만 대리값 | 통과 | 판정                                                      |
| --------------------------- | ---: | -------------------------------------------------- | ---------------------------: | ----------- | ------------: | ---: | --------------------------------------------------------- |
| C1                          |    1 | 무거운 파동 둘(`UG`+`SSGP`+`L`)과 가벼운 셋        |                   17 · 3,050 | 17 · 4,150  |    19 · 2,600 |    0 | 기준이 17건                                               |
| V(a–e)                      |    5 | 여섯 건 동시 또는 2+4·4+2·3+3 배치                 |        (시드 0만) 21 · 6,600 | 21 · 6,600  |      17 · 650 |    0 | 시드 0부터 23건 미달                                      |
| V(f–k)                      |    8 | 냉식 → 싼 온식 → 비싼 온식 정렬, 쌍·단독 간격      |        (시드 0만) 20 · 5,050 | 20 · 5,050  |      17 · 150 |    0 | 같음                                                      |
| V3(현재 포함)               |    6 | 현재 구성과 정렬 변형 × 프렙 5 × 사전 8            |                   23 · 8,200 | 26 · 10,500 |   27 · 12,800 |    0 | 현재 구성: 프렙만이 기준을 넘음                           |
| Pairs(Pa–Pd, slack 냉식 둘) |    4 | (냉식, 온식) 쌍 셋, 같은 tick 또는 10 tick 엇갈림  |                   24 · 8,650 | 25 · 8,900  |    22 · 6,200 |    2 | Pb 둘: 같은 tick 배치가 없고 slack이 사다리 밖(아래 참고) |
| Pairs2(Pa·Pc·Pd·Pe·역순)    |    5 | 같은 tick 쌍 셋, 간격 120–170                      |                   24 · 8,600 | 26 · 11,000 |   25 · 10,000 |    0 | 프렙만이 넘음                                             |
| R1                          |   40 | 무작위: 파동당 2–3 배치, 온식 2–4                  |                   23 · 7,350 | 23 · 9,700  |    21 · 4,500 |    0 | 손익 최솟값 8,500 미달                                    |
| R2(Q·P)                     |   50 | 무작위: 네 파동 8·7·8·7 또는 다섯 파동 (2,2,2)     |                   24 · 8,250 | 24 · 8,700  |    24 · 9,000 |    0 | 프렙만이 넘음                                             |
| R3(Q·P)                     |   80 | 같음                                               |                   24 · 9,600 | 26 · 11,700 |    24 · 8,400 |    5 | P3-019·P3-004: 전수 스윕에서 반증(아래)                   |
| H5·H6                       |   72 | 온식 4–6건 같은 tick 배치를 가진 무거운 파동 2–3개 |                   20 · 4,800 | 22 · 7,000  |    19 · 1,600 |    0 | 지렛대 폭은 넓으나(손익 64단) 기준 상한 19–22건           |
| X7                          |   30 | 무거운 파동 1–2개 + 85 tick 간격 파동              |                   24 · 9,650 | 26 · 12,200 |   26 · 10,700 |    0 | 프렙만이 넘음; 지렛대가 넓은 후보(X7-027)는 21건          |
| YA·YB·YC(후보 3, 32건)      |   96 | 냉식 15–18·온식 14–17, 무거운 파동 둘              |                   25 · 6,650 | 26 · 8,600  |    25 · 6,000 |    0 | 냉식 매출이 낮아 손익 최솟값 6,650–7,750                  |
| Pb 혼합(slack 1 × 8)        |    6 | Pb에 같은 tick 쌍을 하나 이상 넣음                 |                   23 · 8,000 | 27 · 13,100 |    23 · 6,900 |    0 | slack 1 × 8에서는 Pb 자체가 손익 6,500–11,200으로 흔들림  |

무작위 묶음의 시드 0 전용 열은 `RH_RUN`만 있는 V 두 묶음에만 해당하고, 나머지는 여섯 시도 값입니다.
R3에서 대리값으로 통과한 다섯 조합은 두 구성(P3-019 `GPMMSGAUMGAUSPLPLULSRRLGSUAAMR`, 파동마다 같은 tick 쌍 셋 0·170·340; P3-004 `SGLAAGMUMRMGSUAPMSRPSRLLAGUULP`, 0·160·320)에 몰려 있었고, 다섯 조합의 우선순위 사전은 모두 냉식 세 메뉴 0 또는 온식 다섯 메뉴 2(같은 순서)였고, 프렙은 `marinated_protein 3, prepped_grain 3, prepped_vegetable 3, thawed_protein 3`(셋) 또는 `marinated_protein 3, prepped_mushroom 3, prepped_vegetable 4, thawed_protein 2`(P3-019의 둘)였습니다.
P3-019(목표 24건·9,600원 가정, 기준 26 · 25 · 25 · 25 · 25 · 24건)의 시드 0 프렙만 전수 스윕은 3,631행에서 멈췄고 그 안에 24건·9,600원을 넘는 조합이 11개(최고 `prepped_grain 5, prepped_mushroom 6, prepped_vegetable 1, soup_base 2, thawed_protein 1` · 노동량 15 · 25건·9,600원)였으며, P3-004(목표 24건·8,800원 가정, 기준 24 · 28 · 27 · 26 · 24 · 24건)는 3,921행 안에 9개(최고 `prepped_grain 6, prepped_mushroom 1, prepped_vegetable 1, soup_base 3, thawed_protein 1` · 12 · 25건·9,600원)였습니다.
통과가 0이어야 하는 검사에서 부분 스윕의 통과 하나가 곧 반증이므로 둘 다 끝까지 돌리지 않았습니다.

##### 실패의 두 갈래

첫째, 시도 편차를 줄여 주는 우선순위 사전은 모두 "온식을 냉식보다 먼저"라는 한 가지 순서였습니다(`salad 0, grain_salad 0, mushroom_salad 0`과 온식 다섯 메뉴 2는 `_assign_work`의 정렬에서 같은 순서).
이 순서는 냉식대의 손질·조리를 미루어 온식의 긴 사슬을 먼저 태우는 것인데, 프렙은 그 손질 자체를 없애므로 같은 효과를 노동량 12–18로 살 수 있고, 그래서 여섯 시도 최솟값이 23건 이상인 구성마다 시드 0의 프렙만 최댓값이 기준 정책과 나란히 갑니다(P3-004 시도 1에서 프렙만 26건·9,150원 대 기준 26건·9,950원).
`hot_queue`의 채택 후보 E가 피한 함정이 이것이며, 그 영업은 냉식대 하나에 샐러드·수프·구이 둘이 줄을 서는 단일 병목이 있어 우선순위가 줄의 순서를 바꾸고 프렙이 그 순서를 못 바꾸지만, 이 영업은 냉식 둘·화구 둘이라 단일 병목이 없습니다.
둘째, 온식 내부의 순서(구이·덮밥 2, 수프 0 등)가 지렛대가 되는 구성은 같은 tick 온식 4–6건의 포화 배치를 가진 것뿐이고, 그 배치는 파동당 2–3건을 잃으므로 무거운 파동 둘이면 기준 정책의 여섯 시도 최솟값이 19–22건이 되어 올리기만 할 수 있는 제공 목표 23건 아래입니다(H5·H6 72후보 가운데 최솟값 23건 이상은 0).
무거운 파동 하나에 나머지를 85 tick 간격으로 푼 X7과 냉식을 늘린 후보 3(32건) 묶음은 이 둘 사이를 메우려 했으나, 제공이 23건을 지키는 조합은 프렙만이 함께 넘고(X7-015: 기준 최솟값 24건·9,650원, 프렙만 시드 0 26건·10,700원), 지렛대가 넓은 조합은 제공이나 손익 최솟값이 목표 아래였습니다(X7-027 21건, YA8-021 25건·6,650원).

##### slack 주의

Pairs 묶음의 Pb(냉식 먼저·온식 10 tick 뒤 쌍 셋)가 여섯 시도 24–25건·8,650–8,900원으로 유일하게 안정적이었지만, 그 측정은 slack을 `salad 1, grain_salad 1`로 둔 선별이었고 같은 구성을 시작 slack 여덟 메뉴 각 1로 다시 재면 손익이 6,500–11,200원으로 벌어집니다(Pb 혼합 묶음).
§4.4의 사다리는 가장 큰 값을 `menu_ids` 앞쪽부터 0으로 두므로 이 영업에서는 `salad`부터 지워져 냉식만 남는 slack에 닿을 수 없고(`final_service`가 냉식 두 메뉴 slack에 닿은 것은 그 영업의 `menu_ids`가 온식부터 시작하기 때문), Pb는 같은 tick 배치도 없어 `tests/test_m3_content.gd`의 묶음 검사를 통과하지 못하므로 채택 후보가 아닙니다.
곧 이 영업의 시도 편차는 시작 slack에서 재야 하며, 위 표의 값은 Pairs 한 묶음만 빼고 모두 그 slack입니다.
같은 이유로 현재 구성을 냉식 두 메뉴 slack(`salad 1, grain_salad 1`)과 바닥 프렙 `marinated_protein 3, prepped_mushroom 3, prepped_vegetable 4, thawed_protein 3` + 냉식 세 메뉴 0으로 돌리면 여섯 시도가 26 · 26 · 26 · 26 · 25 · 26건으로 고르지만, 그 조합도 시드 0에서 26건·10,500원이라 프렙만 바닥 27건·12,800원(상한 18) 아래이므로 slack 예외가 열리더라도 현재 구성은 후보가 아닙니다.

##### 2차 결론과 열어야 할 규칙

값은 모두 이전 그대로이고(`forecast_slack` 없음, `prep_labor_capacity` 18, `starting_budget` 14,400, 목표 23건·8,500원, 발주 현미 14·양송이 11·연어 7·채소 31, 구성 30건, 브리핑 그대로), `lever_free_policy("rush_hour")`는 Task 10a가 고정한 `move_station cold_01 left` 2회를 유지합니다.
측정이 가리키는 것은 조율 수단이 아니라 검사 규칙의 조합입니다: 프렙만 스윕이 0이어야 하는 2층 검사, 올리기만 하는 제공 목표 23건, 여섯 시도 편차 2건·3,000원, 여덟 메뉴 slack 1이 함께 걸리면 냉식 둘·화구 둘·직원 넷의 이 주방에서는 우선순위가 프렙을 이길 자리가 없습니다.
풀리는 길은 넷 가운데 하나를 운영자가 여는 것입니다: (1) 이 영업의 2층 검사를 `long_route`처럼 스윕 최댓값이 아니라 기준 정책에서 우선순위만 뺀 1층 의미로 읽기(그러면 현재 구성에서도 세 갈래 모두 미달), (2) 제공 목표를 23건 아래로 되돌리는 예외(H5-052 같은 포화 구성은 20건·4,800원에서 프렙만보다 손익 64단 위), (3) 재설계 구성에 한해 slack을 냉식 메뉴로 제한하는 예외(Pb 모양이 그 slack에서 24건·8,650원으로 안정), (4) 지렛대를 프렙 하나로 줄이고 우선순위를 대체 정책으로 내리기.
어느 것도 계획이 이 Task에 준 권한 안에 없으므로 값을 바꾸지 않고 멈춥니다.

#### 2026-09-22 3차: 코드화된 게이트 기준

2026-09-22의 조율 판정으로 위 1차·2차가 요구한 "프렙만 스윕 `passed` 0"은 계획 공통 절차 8이 더한 조건이지 명세 §4.3의 조건이 아니며, 운영자가 승인한 게이트는 `tests/test_m3_playthrough.gd`에 코드화된 두 층뿐입니다: 1층은 기준 정책에서 `set_prep`·`priorities`·둘 다를 뺀 세 정책이 각각 시드 0에서 한 목표 이상에 미달하고, 2층은 프렙도 우선순위도 없는 `lever_free_policy("rush_hour")`가 미달하는 것입니다.
그 기준으로 원래 구성에서 다시 재어 slack 세 메뉴에서 수렴했고, `content/campaign/scenarios/rush_hour.tres`·`tests/fixtures/m3_policies.gd`·`tests/capture_m3.gd`(주석)만 바꿨습니다.
탐색 조건은 (slack 단계, 상한 18–21, 기준 프렙 P, 우선순위 사전 M, 목표 T ≥ 23건·8,500원)가 다음을 함께 만족하는 것입니다: P + M의 여섯 시도 최솟값이 T 이상, 시드 0의 §4.1 여분이 제공 2건·손익 3,000원 이하(곧 시드 0 값과 여섯 시도 최솟값의 편차가 그 안), 무계획이 모든 시도에서 제공 2건 이상 또는 손익 1,500원 이상 미달, 시드 0에서 P만·M만·무계획이 T에 미달, 2층 고정 정책이 T에 미달, 대체 A·B가 T를 통과.
핵심은 "P만의 시드 0 값 < P + M의 여섯 시도 최솟값"이라는 조건이며, 위 1차·2차의 로그 5,791행(단계 1–7 × 프렙 4–5집합 × 두 메뉴 이하 사전 129가지, 세 메뉴 사전 448가지)을 이 조건으로 다시 걸러(`rh9c_filter.py`) 남은 행은 하나뿐이었습니다: 단계 7(`grain_grill 1, protein_bowl 1`), 대체 A의 프렙 `marinated_protein 1, prepped_vegetable 5, prepped_grain 3, thawed_protein 1, prepped_mushroom 3`(노동량 15) + `grill 0, mushroom_soup 0`, 여섯 시도 25/9,400 · 25/9,400 · 25/9,500 · 25/9,300 · 25/10,000 · 25/10,000, 프렙만 시드 0 25건·9,100원.
그 행은 목표 25건·9,300원(제공은 시도 5 무계획 23건·8,200원의 폭 때문에 25, 손익은 프렙만 9,100원을 넘는 9,150–9,300)에서 게이트를 지나지만(scratchpad `rh9c_c6.json`과 같은 꼴의 `rh9c_c7.json`으로 확인: 우선순위만 20건·2,700원, 대체 A는 기준과 같은 프렙이라 `marinated_protein 3, prepped_mushroom 3, prepped_vegetable 4, thawed_protein 2`(27건·12,800원)로 바꿔야 함), 1층 손익 폭이 200원이고 slack이 §4.4의 마지막 단계라 넓은 단계를 먼저 찾았습니다.
2차의 탐침 `rh_candidates.gd`에 현재 구성을 넣고 단계 1–6마다 프렙 17집합(대체 A 프렙과 그 항목별 ±1 변형 10, `soup_base` 1·2를 더한 둘, 노동량 18 이하의 다른 모양 넷) × 두 메뉴 이하 사전 129가지 × 여섯 시도를 쟀습니다(단계마다 2,322행·약 15분, 두 프로세스씩; 로그 `rh9c-r{1..6}.log`, 선별 `rh9c_maps_filter.py`).

| 단계 | slack 1인 메뉴                                                                      | 최솟값 ≥ 23·8,500 | + 편차 2·3,000 이내 | + 프렙만 시드 0 미달 | 최솟값 최고 행                                                                                                                                                                     |
| ---: | ----------------------------------------------------------------------------------- | ----------------: | ------------------: | -------------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|    1 | 여덟 메뉴 전부                                                                      |                 0 |                   0 |                    0 | `marinated_protein 2, prepped_vegetable 5, prepped_grain 3, thawed_protein 1, prepped_mushroom 3` + `mushroom_soup 2, salad 0` · 23/8,400                                          |
|    2 | `salad` 제외 일곱                                                                   |                 0 |                   0 |                    0 | `marinated_protein 1, prepped_vegetable 5, prepped_grain 4, thawed_protein 1, prepped_mushroom 3` + `protein_bowl 2, soup 2` · 24/7,300                                            |
|    3 | `grill`·`grain_salad`·`mushroom_salad`·`mushroom_soup`·`grain_grill`·`protein_bowl` |                 0 |                   0 |                    0 | `marinated_protein 1, prepped_vegetable 5, prepped_grain 3, thawed_protein 2, prepped_mushroom 3` + `mushroom_soup 0, soup 2` · 25/7,650                                           |
|    4 | `grain_salad`·`mushroom_salad`·`mushroom_soup`·`grain_grill`·`protein_bowl`         |                 0 |                   0 |                    0 | 대체 A 프렙 + `grain_grill 0, mushroom_soup 0` · 24/8,250                                                                                                                          |
|    5 | `mushroom_salad`·`mushroom_soup`·`grain_grill`·`protein_bowl`                       |                 0 |                   0 |                    0 | 대체 A 프렙 + `mushroom_soup 0, salad 0` · 24/8,250                                                                                                                                |
|    6 | `mushroom_soup`·`grain_grill`·`protein_bowl`                                        |                32 |                  31 |                   25 | `marinated_protein 1, prepped_vegetable 6, prepped_grain 3, thawed_protein 1, prepped_mushroom 3` + `grain_grill 0, mushroom_salad 0` · 25/9,950(프렙만 시드 0 25/10,100이라 탈락) |
...
그 18행 가운데 시드 0 편차가 가장 작은 행들을 먼저 추리고, 그 안에서 1층 폭(여섯 시도 최솟값 − 프렙만 시드 0)이 가장 넓은 행을 골랐습니다(18행 전체에서는 편차가 더 큰 행 가운데 손익 폭이 더 넓은 행이 있습니다): `marinated_protein 1, prepped_vegetable 5, prepped_grain 3, prepped_mushroom 3, soup_base 2, thawed_protein 1`(노동량 17) + `salad 2, protein_bowl 2`, 여섯 시도 24/9,000 · 26/10,300 · 24/8,850 · 25/9,300 · 24/9,000 · 24/8,800, 편차 0건·200원, 프렙만 시드 0 22건·6,100원(폭 2건·2,700원), 우선순위만 22건·7,000원.
...
| 값                    | 이전         | 이후                                             |
| --------------------- | ------------ | ------------------------------------------------ |
| `forecast_slack`      | 없음         | `mushroom_soup 1, grain_grill 1, protein_bowl 1` |
| `prep_labor_capacity` | 18           | 18                                               |
| 목표                  | 23건·8,500원 | 23건·8,500원                                     |

`maximum_profit(30)`은 17,300원이고, `starting_budget` 14,400, 발주 현미 14·양송이 11·연어 7·채소 31, 구성 30건, 브리핑은 그대로라 `translations/en.po`는 바뀌지 않았습니다.
최종 게이트(`--gate`, 상한 18)는 `passed: true`이고 실패 문장이 없습니다.
...
E의 오프셋은 수프 170–190·두 번째 구이 290–310·마지막 샐러드 400–440·주기 690–700의 아홉 가지를 같은 스윕으로 쟀고, 여섯 변형에서 시도 5의 최댓값이 14건·5,750원으로 같았으며(다섯은 프렙 없음, 수프 190·구이 310은 `soup_base 1`) 나머지 셋(두 번째 구이 290 둘, 주기 700)은 13건에 그쳤습니다.
...
여섯 시도 교집합은 그 여섯 변형 모두에서 비지 않았고 크기는 수프 170·구이 300이 1, 190·300이 1, 180·310이 3, 180·300·샐러드 400이 2, 180·300·샐러드 440이 1, 190·310이 7이며, 기준 프렙 `marinated_protein 1, prepped_vegetable 3`은 180·300·샐러드 400을 뺀 다섯에 들어 있습니다.
그 다섯 가운데 180·300·샐러드 440은 시드 0이 17건·9,500원이라 §4.1 여분 3건으로 빠지고, 남은 넷은 §4.1로는 갈리지 않아(모두 시드 0 16건·8,000원) 수프 180·구이 310을 골랐습니다: 기준 프렙이 시도 1–4에서 15건 이상을 유지하는 변형은 170·300과 이것 둘뿐이고(190·300과 190·310은 시도 1·2에서 14건), 둘 가운데 이것이 시도 2에서 17건·7,800원(170·300은 15건·6,800원)으로 높으며, 교집합에 노동량 1인 `prepped_grain 1`·`soup_base 1`까지 함께 드는 유일한 변형입니다.
앞으로 다시 조율할 때 먼저 닿는 경계는 셋입니다: 시도 5의 기준 정책 14건·5,750원 대 목표 14건·5,600원(제공 여유 0건·손익 150원), 시드 0의 §4.1 제공 여분이 정확히 2건, 우선순위 없는 첫 파동 구이의 제공 tick 510이 마감과 같은 것(`service_sim.gd`의 `step()`이 만료 검사를 작업 완료보다 먼저 돌려 마감 tick의 제공은 만료).
프렙 없는 정책 `{}` + `grill 2`는 시도 0·2·3·4·5를 통과하고(16건·8,600원, 14건·5,900원, 15건·7,850원, 14건·5,600원, 14건·5,750원; 시드 0에서는 기준의 8,000원보다 많음) 시도 1(13건·5,250원)에서만 미달하므로, 브리핑이 프렙을 권하는 근거는 그 한 시도입니다.
