...
프렙 노동 한도와 정책 기준이 바뀌었으므로 콘텐츠 버전 3에서 진행 중인 영업은 이전 준비 선택을 복원하지 않고 다시 시작합니다.
...
프렙 노동 한도를 12에서 6으로 줄여 구이 4개에 노동을 모두 쓰는 선택을 거부하고, 안내 문구는 구이와 빠른 메뉴에 프렙을 나누도록 설명합니다.
`tests/test_m3_playthrough.gd`는 배치 명령 없이 프렙과 메뉴 우선순위만으로 목표를 통과하고 1배속·4배속의 최종 상태 해시가 같은지 검사합니다.
`tests/test_m4_store.gd`는 콘텐츠 버전 3 기록을 유지하고 활성 영업만 재시작한 뒤 다음 쓰기에서 버전 4로 갱신하는지 검사합니다.

`GODOT_BIN=/Applications/Godot.app/Contents/MacOS/Godot bash scripts/check.sh m2`는 487개 검사를 실패 없이 통과했습니다.
`GODOT_BIN=/Applications/Godot.app/Contents/MacOS/Godot bash scripts/check.sh m3`는 1,049개 검사를 실패 없이 통과했습니다.
`GODOT_BIN=/Applications/Godot.app/Contents/MacOS/Godot bash scripts/check.sh m4`는 1,183개 검사를 실패 없이 통과했습니다.
