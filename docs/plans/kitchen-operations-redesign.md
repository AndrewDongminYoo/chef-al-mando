...
2026-09-11 운영자는 사람에게 다시 QA를 요청하기 전에 자동 정책으로 도달 가능성과 난이도 차이를 확인하도록 승인했습니다.

- 후반 여섯 영업마다 기준 정책과 결과가 다른 대체 정책이 목표를 통과해야 합니다.
- 두 정책은 반복 실행과 1배속·4배속 실행에서 각각 같은 최종 상태 해시를 만들어야 합니다.
- 무계획 정책은 목표보다 제공 수가 2건 이상 부족하거나 손익이 1,500 이상 부족해야 합니다.
- `split_duties`의 마지막 주문 파동은 25 tick 앞당겨 무계획 정책과 준비·담당 정책의 차이를 확대합니다.
- `hot_queue`는 구이 프렙 3개와 4개 정책을 모두 허용하도록 목표 손익을 4,750으로 조정합니다.
- 새 쓰기의 콘텐츠 버전을 3으로 올립니다. 버전 1·2의 완료·최고 기록은 보존하고 버전 2의 진행 중 영업도 다시 시작합니다.
