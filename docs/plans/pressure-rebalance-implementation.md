...
- PR #30 호스티드 Codex 리뷰 라운드 2 P2(수정 커밋 `fix(persistence): grant a legacy completion only against target pairs shipped by the save's content version`): 바로 위 라운드가 도입한 쌍 목록은 문서 자신의 `content_version`을 무시해, 콘텐츠 1 저장의 `hot_queue` 완료가 콘텐츠 3에서야 나온 12건·4,750 쌍까지 주장해 legacy_completed로 잠금 해제할 수 있었으므로, 각 쌍에 그 쌍을 도입한 콘텐츠 버전(`since_content`)을 붙이고 `meets_legacy_completion_targets(scenario_id, record, content_version: int = -1)`가 `since_content <= content_version`인 쌍만 세도록 고쳤습니다(`sim/campaign_progress.gd`).
  `persistence/campaign_store.gd`의 `_read()`는 이제 `int(document.content_version)`을 넘기고, `validate_records()`(저장을 발급한 문서 버전을 모르는 기록)는 기본값 −1로 모든 쌍을 그대로 셉니다(명세의 2026-09-22 정정 절 7).
